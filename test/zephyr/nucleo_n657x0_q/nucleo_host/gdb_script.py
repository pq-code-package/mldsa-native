...
def build_run_script(
    *,
    ...
    C startup until Zephyr calls ``get_bootargs()``, restore bootargs after
    ``.bss`` has been cleared, install completion/fault breakpoints, continue,
    then read the target return code from the completion breakpoint.
    """
    gdb_lines = [
        "set pagination off",
        "set confirm off",
        ...
        "load",
        f"tbreak {bootargs_break}",
        f"jump {reset_handler_jump}",
        restore_bootargs_command(argv_bin, arg_block_addr, arg_block_sym),
...
def restore_bootargs_command(argv_bin, arg_block_addr, arg_block_sym):
    """Return the GDB ``restore`` command for the bootargs string."""
    if arg_block_addr:
