...
#### Building for m33-an521 (Recommended)

The recommended way to build for m33-an521 is using the platform makefile:

```bash
# C backend (portable)
make lib EXTRA_MAKEFILE=test/baremetal/platform/m33-an521/platform.mk OPT=0

# DSP backend (optimized, when available)
make lib EXTRA_MAKEFILE=test/baremetal/platform/m33-an521/platform.mk OPT=1
```
...
# Step 2: Run tests
cd test/baremetal/platform/m33-an521
make run_func_all
make run_bench_all
```
