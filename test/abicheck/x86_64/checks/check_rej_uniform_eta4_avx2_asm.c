...
int check_rej_uniform_eta4_avx2_asm(void)
{
  ...
  int violations;
  MLD_ALIGN uint8_t
      buf_rdi[1024]; /* Output coefficient buffer (256 x int32_t) */
  MLD_ALIGN uint8_t buf_rdx[2048]; /* Lookup table (256 x 8 uint8_t =
                                      mld_rej_uniform_table) */
  MLD_ALIGN uint8_t buf_rsi[272];  /* Input byte buffer
                                      (MLD_AVX2_REJ_UNIFORM_ETA4_BUFLEN = 272) */

  if (!mld_sys_check_capability(MLD_SYS_CAP_X86_64_AVX2))
  {
    fprintf(stderr,
            "ABI check rej_uniform_eta4_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for rej_uniform_eta4_avx2_asm (iteration %d): "
              "%d violations\n",
