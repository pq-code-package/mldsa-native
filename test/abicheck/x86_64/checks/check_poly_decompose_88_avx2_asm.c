...
int check_poly_decompose_88_avx2_asm(void)
{
  ...
  int violations;
  MLD_ALIGN uint8_t
      buf_rdi[1024]; /* Output high-part polynomial (256 x int32_t) */
  MLD_ALIGN uint8_t
      buf_rsi[1024]; /* Input polynomial / output low-part (256 x int32_t) */

  if (!mld_sys_check_capability(MLD_SYS_CAP_X86_64_AVX2))
  {
    fprintf(
        stderr,
        "ABI check poly_decompose_88_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_decompose_88_avx2_asm (iteration %d): "
              "%d violations\n",
