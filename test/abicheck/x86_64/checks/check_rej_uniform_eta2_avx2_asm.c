...
int check_rej_uniform_eta2_avx2_asm(void)
{
  ...
  MLD_ALIGN uint8_t buf_rsi[136];  /* Input byte buffer
                                      (MLD_AVX2_REJ_UNIFORM_ETA2_BUFLEN = 136) */

  if (!mld_sys_check_capability(MLD_SYS_CAP_X86_64_AVX2))
  {
    fprintf(stderr,
            "ABI check rej_uniform_eta2_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for rej_uniform_eta2_avx2_asm (iteration %d): "
              "%d violations\n",
