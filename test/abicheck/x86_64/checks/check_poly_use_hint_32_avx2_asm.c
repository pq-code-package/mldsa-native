...
int check_poly_use_hint_32_avx2_asm(void)
{
  ...
  MLD_ALIGN uint8_t buf_rsi[1024]; /* Hint polynomial (256 x int32_t) */

  if (!mld_sys_check_capability(MLD_SYS_CAP_X86_64_AVX2))
  {
    fprintf(stderr,
            "ABI check poly_use_hint_32_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_use_hint_32_avx2_asm (iteration %d): "
              "%d violations\n",
