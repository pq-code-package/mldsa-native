...
MLD_SYSV_ABI
unsigned mld_rej_uniform_avx2_asm(int32_t *r, const uint8_t buf[840],
...
int check_rej_uniform_avx2_asm(void)
{
  ...
  int violations;
  MLD_ALIGN uint8_t buf_rdi[1024]; /* Output buffer (256 x int32_t) */
  MLD_ALIGN uint8_t buf_rdx[2048]; /* Lookup table (256 x 8 x uint8_t) */
  MLD_ALIGN uint8_t buf_rsi[840];  /* Input buffer (MLD_POLY_UNIFORM_NBLOCKS *
                                      SHAKE128_RATE = 5 * 168) */

  if (!mld_sys_check_capability(MLD_SYS_CAP_X86_64_AVX2))
  {
    fprintf(stderr,
            "ABI check rej_uniform_avx2_asm: host lacks AVX2, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for rej_uniform_avx2_asm (iteration %d): %d "
              "violations\n",
