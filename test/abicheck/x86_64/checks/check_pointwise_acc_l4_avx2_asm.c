...
MLD_SYSV_ABI
void mld_pointwise_acc_l4_avx2_asm(int32_t *c, const int32_t a[4][256],
                                   const int32_t b[4][256],
                                   const int32_t *qdata);
...
int check_pointwise_acc_l4_avx2_asm(void)
{
  ...
  MLD_ALIGN uint8_t
      buf_rdx[4096]; /* Input polynomial vector b (4 x 256 x int32_t) */
  MLD_ALIGN uint8_t
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_rdx, 4096);
    randombytes(buf_rsi, 4096);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for pointwise_acc_l4_avx2_asm (iteration %d): "
              "%d violations\n",
