...
void mld_intt_rv32im_slowmul_asm(int32_t r[256], const int32_t zetas[510]);
...
int check_intt_rv32im_slowmul_asm(void)
{
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    call_stub_riscv32(&input_state, &output_state,
                      (void (*)(void))mld_intt_rv32im_slowmul_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for intt_rv32im_slowmul_asm (iteration %d): %d "
              "violations\n",
