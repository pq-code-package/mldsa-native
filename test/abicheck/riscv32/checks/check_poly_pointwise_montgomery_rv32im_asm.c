...
void mld_poly_pointwise_montgomery_rv32im_asm(int32_t a[256],
                                              const int32_t b[256]);
...
int check_poly_pointwise_montgomery_rv32im_asm(void)
{
  ...
  MLD_ALIGN uint8_t buf_a1[1024]; /* Input polynomial (256 x int32_t) */
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    call_stub_riscv32(&input_state, &output_state,
                      (void (*)(void))mld_poly_pointwise_montgomery_rv32im_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_pointwise_montgomery_rv32im_asm "
