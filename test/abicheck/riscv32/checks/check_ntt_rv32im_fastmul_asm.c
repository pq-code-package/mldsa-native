...
void mld_ntt_rv32im_fastmul_asm(int32_t r[256], const int32_t zetas[510]);
...
int check_ntt_rv32im_fastmul_asm(void)
{
  ...
  MLD_ALIGN uint8_t buf_a1[2040]; /* Twiddle table (255 Barrett pairs) */
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    call_stub_riscv32(&input_state, &output_state,
                      (void (*)(void))mld_ntt_rv32im_fastmul_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for ntt_rv32im_fastmul_asm (iteration %d): %d "
              "violations\n",
