...
int check_polyvecl_pointwise_acc_montgomery_l5_aarch64_asm(void)
{
  ...
  MLD_ALIGN uint8_t
      buf_x1[5120]; /* Input polynomial vector a (5 x 256 x int32_t) */
  MLD_ALIGN uint8_t
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    randombytes(buf_x1, 5120);
    randombytes(buf_x2, 5120);
    ...
    if (violations > 0)
    {
      fprintf(stderr,
              "ABI test FAILED for "
              "polyvecl_pointwise_acc_montgomery_l5_aarch64_asm (iteration "
              "%d): %d violations\n",
