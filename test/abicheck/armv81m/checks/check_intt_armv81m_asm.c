...
void mld_intt_armv81m_asm(int32_t r[256]);
...
int check_intt_armv81m_asm(void)
{
  ...
  {
    fprintf(stderr,
            "ABI check intt_armv81m_asm: host lacks Armv8.1-M MVE, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    call_stub_armv81m(&input_state, &output_state,
                      (void (*)(void))mld_intt_armv81m_asm);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for intt_armv81m_asm (iteration %d): %d "
              "violations\n",
