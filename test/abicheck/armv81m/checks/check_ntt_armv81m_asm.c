...
void mld_ntt_armv81m_asm(int32_t r[256]);
...
int check_ntt_armv81m_asm(void)
{
  ...
  int violations;
  MLD_ALIGN uint8_t
      buf_r0[1024]; /* 256-coefficient input/output polynomial; kernel output is
                       pqmx 4-by-4-transposed NTT order */

  if (!mld_sys_check_capability(MLD_SYS_CAP_ARMV81M_MVE))
  {
    fprintf(stderr,
            "ABI check ntt_armv81m_asm: host lacks Armv8.1-M MVE, skipping\n");
  ...
  for (test_iter = 0; test_iter < MLD_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    if (violations > 0)
    {
      fprintf(
          stderr,
          "ABI test FAILED for ntt_armv81m_asm (iteration %d): %d violations\n",
