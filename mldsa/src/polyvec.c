...
uint32_t mld_polyveck_unpack_eta_chknorm(
    const uint8_t r[MLDSA_K * MLDSA_POLYETA_PACKEDBYTES], int32_t bound,
    mld_poly *scratch)
{
  unsigned int i;
  uint32_t t = 0;
  ...
  for (i = 0; i < MLDSA_K; i++)
  __loop__(
    assigns(i, t, memory_slice(scratch, sizeof(mld_poly)))
    invariant(i <= MLDSA_K)
    invariant(t == 0 || t == 0xFFFFFFFF)
    decreases(MLDSA_K - i)
  )
  {
    mld_polyeta_unpack(scratch, r + i * MLDSA_POLYETA_PACKEDBYTES);
    /* Reference: Leaks which polynomial violates the bound via a conditional.
     * We are more conservative to reduce the number of declassifications in
     * constant-time testing.
     */
    t |= mld_poly_chknorm(scratch, bound);
  }
