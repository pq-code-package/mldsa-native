...
void mld_unpack_sk(uint8_t rho[MLDSA_SEEDBYTES], uint8_t tr[MLDSA_TRBYTES],
                   uint8_t key[MLDSA_SEEDBYTES], mld_t0vec *t0, mld_s1vec *s1,
                   mld_s2vec *s2, const uint8_t sk[MLDSA_CRYPTO_SECRETKEYBYTES])
{
                         ...
                         unsigned int k, unsigned int *hints_written)
{
  ...
  for (j = 0; j < MLDSA_N; j++)
  __loop__(
    assigns(j, object_whole(hints_written),
            memory_slice(sig_h, MLDSA_POLYVECH_PACKEDBYTES))
    invariant(j <= MLDSA_N)
    decreases(MLDSA_N - j)
  )
  {
    if (h->coeffs[j] != 0 && *hints_written < MLDSA_OMEGA)
    {
      sig_h[*hints_written] = (uint8_t)j;
      (*hints_written)++;
    }
  }
  sig_h[MLDSA_OMEGA + k] = (uint8_t)*hints_written;
}
