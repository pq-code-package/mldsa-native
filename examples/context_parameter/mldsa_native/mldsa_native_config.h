...
/*
 * Test configuration: Configuration with a context parameter and custom
 * allocation
 *
 ...
 */
#define MLD_CONFIG_CONTEXT_PARAMETER_TYPE example_context *
...
#define MLD_CUSTOM_ALLOC(v, T, N, context) \
  T *v = example_context_malloc((context), sizeof(T) * (N))
#define MLD_CUSTOM_FREE(v, T, N, context) \
  example_context_free((context), (v), sizeof(T) * (N))
#endif /* !__ASSEMBLER__ */
