...
secret-independent timing, with the rejection samplers as special cases. The
matrix sampler `rej_uniform` operates on public data only and is deliberately
variable-time. The secret-vector samplers `rej_uniform_eta{2,4}` are proved to
have secret-independent timing: their event trace depends only on the public
pointers and on which candidates are accepted or rejected, never on the
accepted coefficient values. That reject pattern is safe to leak, being
statistically independent of those values. See
