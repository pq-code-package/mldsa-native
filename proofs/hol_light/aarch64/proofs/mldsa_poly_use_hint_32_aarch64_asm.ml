...
let poly_use_hint_32_aarch64_asm_mc = define_assert_from_elf
 "poly_use_hint_32_aarch64_asm_mc" "aarch64/mldsa/mldsa_poly_use_hint_32_aarch64_asm.o"
(*** BYTECODE START ***)
[
...
let MLDSA_POLY_USE_HINT_32_EXEC = ARM_MK_EXEC_RULE poly_use_hint_32_aarch64_asm_mc;;
...
let MLDSA_POLY_USE_HINT_32_CORRECT = prove
 (`!a h x y pc.
    nonoverlapping (word pc, LENGTH poly_use_hint_32_aarch64_asm_mc) (a, 1024) /\
    ...
    ==> ensures arm
          ...
          (\s. read PC s = word(pc + LENGTH poly_use_hint_32_aarch64_asm_mc - 4) /\
  ...
  MAP_EVERY (fun n -> ARM_STEPS_TAC MLDSA_POLY_USE_HINT_32_EXEC [n] THEN
                      SIMD_SIMPLIFY_TAC[])
        (1--878) THEN
  ...
  (* Pre-rewrite mldsa_use_hint_32 -> _code via the equivalence at all
     occurrences. Then split into per-element leaves: FIPS-eq via EC_FINAL,
     val<16 via direct MOD reduction on mldsa_use_hint_32_code. Index-bound
     side conditions left over from IMP_REWRITE_TAC close via ARITH. *)
...
(* Subroutine form: derives directly from MLDSA_POLY_USE_HINT_32_CORRECT
...
let full_spec,public_vars = mk_safety_spec
    ...
    MLDSA_POLY_USE_HINT_32_SUBROUTINE_CORRECT
...
let MLDSA_POLY_USE_HINT_32_SUBROUTINE_SAFE = time prove
 (`exists f_events.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars MLDSA_POLY_USE_HINT_32_EXEC);;
