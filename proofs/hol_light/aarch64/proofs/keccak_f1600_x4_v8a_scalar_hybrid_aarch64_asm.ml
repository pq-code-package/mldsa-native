...
(* Corresponding s2n-bignum proof: arm/proofs/sha3_keccak4_f1600.ml          *)
(* ========================================================================= *)
...
(**** print_literal_from_elf "aarch64/mldsa/keccak_f1600_x4_v8a_scalar_hybrid_aarch64_asm.o";;
 ****)

let keccak_f1600_x4_v8a_scalar_mc = define_assert_from_elf
  "keccak_f1600_x4_v8a_scalar_mc" "aarch64/mldsa/keccak_f1600_x4_v8a_scalar_hybrid_aarch64_asm.o"
(*** BYTECODE START ***)
[
...
let LENGTH_KECCAK_F1600_X4_V8A_SCALAR_HYBRID_MC =
...
let LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV[LENGTH_KECCAK_F1600_X4_V8A_SCALAR_HYBRID_MC;
...
let KECCAK_F1600_X4_V8A_SCALAR_HYBRID_CORRECT = prove
 (`!a rc A1 A2 A3 A4 pc stackpointer.
  ...
  ENSURES_WHILE_PAUP_TAC `1` `12` `pc + 0x718` `pc + 0xd20`
   `\i s.
      (read SP s = stackpointer /\
       wordlist_from_memory(rc,24) s = round_constants /\
...
let KECCAK_F1600_X4_V8A_SCALAR_HYBRID_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a rc pc stackpointer returnaddress.
           ...
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) keccak_f1600_x4_v8a_scalar_mc /\
                    read PC s = word pc /\
                    read SP s = stackpointer /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [a; rc] s /\
