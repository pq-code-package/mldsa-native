...
let poly_decompose_32_aarch64_asm_mc = define_assert_from_elf "poly_decompose_32_aarch64_asm_mc" "aarch64/mldsa/mldsa_poly_decompose_32_aarch64_asm.o"
(*** BYTECODE START ***)
[
...
let LENGTH_MLDSA_POLY_DECOMPOSE_32_MC =
  REWRITE_CONV[poly_decompose_32_aarch64_asm_mc] `LENGTH poly_decompose_32_aarch64_asm_mc`
  |> CONV_RULE (RAND_CONV LENGTH_CONV);;

let MLDSA_POLY_DECOMPOSE_32_CORE_START = new_definition
  `MLDSA_POLY_DECOMPOSE_32_CORE_START = 0`;;

let MLDSA_POLY_DECOMPOSE_32_POSTAMBLE_LENGTH = new_definition
  `MLDSA_POLY_DECOMPOSE_32_POSTAMBLE_LENGTH = 4`;;

let MLDSA_POLY_DECOMPOSE_32_CORE_END = new_definition
  `MLDSA_POLY_DECOMPOSE_32_CORE_END =
     LENGTH poly_decompose_32_aarch64_asm_mc - MLDSA_POLY_DECOMPOSE_32_POSTAMBLE_LENGTH`;;

let LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV[LENGTH_MLDSA_POLY_DECOMPOSE_32_MC;
              MLDSA_POLY_DECOMPOSE_32_CORE_START; MLDSA_POLY_DECOMPOSE_32_CORE_END;
 ...
 `!pc a r1 x.
    ...
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) poly_decompose_32_aarch64_asm_mc /\
              read PC s = word(pc + MLDSA_POLY_DECOMPOSE_32_CORE_START) /\
              C_ARGUMENTS [r1; a] s /\
              (!i. i < 256
                   ==> read(memory :> bytes32(word_add a (word(4 * i)))) s =
                       x i))
         (\s. read PC s = word(pc + MLDSA_POLY_DECOMPOSE_32_CORE_END) /\
  ...
  REWRITE_TAC[NONOVERLAPPING_CLAUSES; C_ARGUMENTS; SOME_FLAGS;
              fst MLDSA_POLY_DECOMPOSE_32_EXEC;
  ...
  MAP_UNTIL_TARGET_PC (fun n ->
    ARM_STEPS_TAC MLDSA_POLY_DECOMPOSE_32_EXEC [n] THEN
 ...
 `!pc a r1 x returnaddress.
  ...
  MP_TAC(REWRITE_RULE[NONOVERLAPPING_CLAUSES; C_ARGUMENTS; SOME_FLAGS;
   ...
   (SPECL [`pc:num`; `a:int64`; `r1:int64`; `x:num->int32`]
    (CONV_RULE LENGTH_SIMPLIFY_CONV MLDSA_POLY_DECOMPOSE_32_CORRECT))) THEN
  ANTS_TAC THENL [ASM_REWRITE_TAC[]; ALL_TAC] THEN
  ARM_BIGSTEP_TAC MLDSA_POLY_DECOMPOSE_32_EXEC "s1" THEN
  ARM_STEPS_TAC MLDSA_POLY_DECOMPOSE_32_EXEC [2] THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  DISCH_TAC THEN
...
let full_spec,public_vars = mk_safety_spec
    ...
    MLDSA_POLY_DECOMPOSE_32_SUBROUTINE_CORRECT
