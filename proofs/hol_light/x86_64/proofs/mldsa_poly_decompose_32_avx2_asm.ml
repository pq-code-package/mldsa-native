...
(* ========================================================================= *)
(* Functional correctness of poly_decompose_32 (x86_64 AVX2):                *)
(* Decompose polynomial coefficients into (a1, a0) where                     *)
(* a mod^+ Q = a1*2*GAMMA2 + a0 for GAMMA2 = (Q-1)/32 = 261888               *)
(* (ML-DSA-65 / ML-DSA-87).                                                  *)
(*                                                                           *)
...
(**** print_literal_from_elf "x86_64/mldsa/mldsa_poly_decompose_32_avx2_asm.o";;
 ****)

let mldsa_decompose32_mc = define_assert_from_elf "mldsa_decompose32_mc" "x86_64/mldsa/mldsa_poly_decompose_32_avx2_asm.o"
(*** BYTECODE START ***)
[
      ...
      (word_ushr
       (word_mul
        ...
        (word_subword
         ...
         (word 1025:int32)) (16,16):16 word):int32)
       (word 512:int32)) 14)
...
(* The unsigned high-16 multiply (VPMULHUW) on a sub-2^16 lane, specialized
   to the Barrett magic constant 1025 from the shared MULHI_LANE_GEN. *)
let MULHI_LANE = prove(
  `!t:int32. val t < 65536 ==>
     val(word_subword (word_mul (word_zx (word_subword t (0,16):16 word):int32)
                                (word 1025)) (16,16):16 word) =
     (val t * 1025) DIV 65536`,
...
let H_NUM = prove(
  ...
  SUBGOAL_THEN `val(m:16 word) = ((val(x:int32) + 127) DIV 128 * 1025) DIV 65536` ASSUME_TAC THENL
   [EXPAND_TAC "m" THEN
  ...
  SUBGOAL_THEN `val(m:16 word) < 1025` ASSUME_TAC THENL
  ...
  SUBGOAL_THEN `((val(x:int32) + 127) DIV 128 * 1025) DIV 65536 < 1025` ASSUME_TAC THENL
   [ASM_MESON_TAC[]; ALL_TAC] THEN
  ABBREV_TAC `q = ((val(x:int32) + 127) DIV 128 * 1025) DIV 65536` THEN
  SUBGOAL_THEN `(q * 512) MOD 4294967296 = q * 512` SUBST1_TAC THENL
   [MATCH_MP_TAC MOD_LT THEN ASM_ARITH_TAC; ALL_TAC] THEN
  SUBGOAL_THEN `(q * 512) DIV 16384 < 33` ASSUME_TAC THENL
   [SIMP_TAC[RDIV_LT_EQ; ARITH_RULE `~(16384 = 0)`] THEN ASM_ARITH_TAC; ALL_TAC] THEN
  SUBGOAL_THEN `((q * 512) DIV 16384 + 1) MOD 4294967296 = (q * 512) DIV 16384 + 1` SUBST1_TAC THENL
   [MATCH_MP_TAC MOD_LT THEN ASM_ARITH_TAC; ALL_TAC] THEN
  SUBGOAL_THEN `((q * 512) DIV 16384 + 1) DIV 2 < 65536` ASSUME_TAC THENL
   [ASM_ARITH_TAC; ALL_TAC] THEN
  ASM_SIMP_TAC[MOD_LT; ARITH_RULE `n < 65536 ==> n < 4294967296`]);;

(* Collapse the two trailing right-shift floors into one. *)
let ADD32_DIV = prove(
  `!c1. (c1 + 32) DIV 32 = c1 DIV 32 + 1`,
  GEN_TAC THEN SUBGOAL_THEN `c1 + 32 = c1 + 1 * 32` SUBST1_TAC THENL
   [ARITH_TAC; SIMP_TAC[DIV_MULT_ADD; ARITH_RULE `~(32 = 0)`]]);;

let DIVMUL_HELP = prove(`!a. (a * 512) DIV 16384 = a DIV 32`,
  GEN_TAC THEN REWRITE_TAC[ARITH_RULE `16384 = 512 * 32`] THEN
  GEN_REWRITE_TAC (LAND_CONV o RAND_CONV) [MULT_SYM] THEN
  ONCE_REWRITE_TAC[MULT_SYM] THEN
  SIMP_TAC[DIV_MULT2; ARITH_RULE `~(512 = 0)`]);;

let E_COLLAPSE = prove(
  `!c. ((((c * 1025) DIV 65536) * 512) DIV 16384 + 1) DIV 2 =
       ((c * 1025) DIV 65536 + 32) DIV 64`,
  GEN_TAC THEN REWRITE_TAC[DIVMUL_HELP; GSYM ADD32_DIV] THEN
  REWRITE_TAC[ARITH_RULE `64 = 32 * 2`; DIV_DIV]);;

(* Barrett-style correctness of the mulhi/mulhrs quotient. *)
let BARRETT_CORE = prove(
  `!c. c < 65473 ==> ((c * 1025) DIV 65536 + 32) DIV 64 = (c + 2045) DIV 4092`,
  GEN_TAC THEN DISCH_TAC THEN
  ABBREV_TAC `c1 = (c * 1025) DIV 65536` THEN
  MP_TAC(SPECL [`c * 1025`; `65536`] DIVISION) THEN ANTS_TAC THENL [ARITH_TAC; ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN STRIP_TAC THEN
  MATCH_MP_TAC DIV_BOUNDS_EQ THEN CONV_TAC NUM_REDUCE_CONV THEN
  MP_TAC(SPECL [`c + 2045`; `4092`] DIVISION) THEN ANTS_TAC THENL [ARITH_TAC; ALL_TAC] THEN
  ABBREV_TAC `q = (c + 2045) DIV 4092` THEN
  ABBREV_TAC `t = (c + 2045) MOD 4092` THEN STRIP_TAC THEN
  SUBGOAL_THEN `q <= 16` ASSUME_TAC THENL
   [SUBGOAL_THEN `q * 4092 <= c + 2045` MP_TAC THENL [ASM_ARITH_TAC; ASM_ARITH_TAC]; ALL_TAC] THEN
  ASM_ARITH_TAC);;

(* round-half-down closed form. *)
let ROUND_CLOSED = prove(
  `!r. (if r MOD 523776 * 2 <= 523776 then r DIV 523776 else r DIV 523776 + 1) =
       (r + 261887) DIV 523776`,
  GEN_TAC THEN
  MP_TAC(SPECL [`r:num`; `523776`] DIVISION) THEN ANTS_TAC THENL [ARITH_TAC; STRIP_TAC] THEN
  ABBREV_TAC `q = r DIV 523776` THEN ABBREV_TAC `m = r MOD 523776` THEN
  COND_CASES_TAC THENL
   [SUBGOAL_THEN `(r + 261887) DIV 523776 = q` (fun th -> REWRITE_TAC[th]) THEN
    MATCH_MP_TAC DIV_UNIQ THEN EXISTS_TAC `m + 261887` THEN ASM_ARITH_TAC;
    SUBGOAL_THEN `261889 <= m` ASSUME_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
    SUBGOAL_THEN `(r + 261887) DIV 523776 = q + 1` (fun th -> REWRITE_TAC[th]) THEN
    MATCH_MP_TAC DIV_UNIQ THEN EXISTS_TAC `m - 261889` THEN ASM_ARITH_TAC]);;

let C_TO_R = prove(
  `!r. (((r + 127) DIV 128) + 2045) DIV 4092 = (r + 261887) DIV 523776`,
  GEN_TAC THEN
  SUBGOAL_THEN `(r + 127) DIV 128 + 2045 = (r + 261887) DIV 128` SUBST1_TAC THENL
   [SUBGOAL_THEN `r + 261887 = (r + 127) + 2045 * 128` SUBST1_TAC THENL
     [ARITH_TAC; SIMP_TAC[DIV_MULT_ADD; ARITH_RULE `~(128 = 0)`]]; ALL_TAC] THEN
  REWRITE_TAC[DIV_DIV] THEN REWRITE_TAC[ARITH_RULE `128 * 4092 = 523776`]);;

let H_ROUND = prove(
  `!r. r < 8380417 ==>
     (((((r + 127) DIV 128 * 1025) DIV 65536) * 512) DIV 16384 + 1) DIV 2 =
     (if r MOD 523776 * 2 <= 523776 then r DIV 523776 else r DIV 523776 + 1)`,
  GEN_TAC THEN DISCH_TAC THEN
...
(* The wrap-around test in word form, specialized to the threshold 8118528
   from the shared IGT_BOUND_GEN. *)
let IGT_BOUND =
  GEN_ALL(MP (SPECL [`x:int32`; `8118528`] IGT_BOUND_GEN)
...
(* On wrap-around (val x > 31*GAMMA2) the rounding quotient is exactly 16. *)
let ROUND32_SPECIAL = prove(
  ...
  ASM_CASES_TAC `n < 8380416` THENL
   [SUBGOAL_THEN `n DIV 523776 = 15` ASSUME_TAC THENL
    ...
    COND_CASES_TAC THENL
     [MP_TAC(SPECL [`n:num`; `523776`] DIVISION) THEN ANTS_TAC THENL [ARITH_TAC; ALL_TAC] THEN
    ...
    COND_CASES_TAC THENL
     [SUBGOAL_THEN `val(x:int32) <= 8118528` ASSUME_TAC THENL
      ...
      SUBGOAL_THEN `~(val(x:int32) DIV 523776 = 16)` ASSUME_TAC THENL
       [DISCH_TAC THEN MP_TAC(SPECL [`val(x:int32)`; `523776`] DIVISION) THEN
  ...
  COND_CASES_TAC THENL
   [SUBGOAL_THEN `h = 16` SUBST1_TAC THENL
    ...
    SUBGOAL_THEN `~(h = 16)` ASSUME_TAC THENL
     [DISCH_TAC THEN
      SUBGOAL_THEN `val(x:int32) <= 8118528` ASSUME_TAC THENL [ASM_ARITH_TAC; ALL_TAC] THEN
 ...
 `!a1 a (x:num->int32) pc.
        ...
        ==> ensures x86
             ...
             (\s. read RIP s = word(pc + 2143) /\
                  ...
                  (!i. i < 256
                       ==> val(read(memory :> bytes32(word_add a1 (word(4*i)))) s) <= 15) /\
                  (!i. i < 256
                       ==> --(&261888) <=
                           ival(read(memory :> bytes32(word_add a (word(4*i)))) s) /\
                           ival(read(memory :> bytes32(word_add a (word(4*i)))) s) <= &261888))
...
let MLDSA_POLY_DECOMPOSE_32_NOIBT_SUBROUTINE_CORRECT = prove(
...
let MLDSA_POLY_DECOMPOSE_32_SAFE = time prove
 (full_spec,
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars MLDSA_POLY_DECOMPOSE_32_EXEC);;

let MLDSA_POLY_DECOMPOSE_32_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a1 a pc stackpointer returnaddress.
          ...
          ==> ensures x86
               ...
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mldsa_decompose32_tmc MLDSA_POLY_DECOMPOSE_32_SAFE THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let MLDSA_POLY_DECOMPOSE_32_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a1 a pc stackpointer returnaddress.
          aligned 32 a1 /\ aligned 32 a /\
          ALL (nonoverlapping (word pc, LENGTH mldsa_decompose32_mc))
               ...
               (\s. read RIP s = returnaddress /\
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [a,1024; a1,1024; a,1024; stackpointer,8]
                                               [a1,1024; a,1024; stackpointer,8]))
               (\s s'. true)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLDSA_POLY_DECOMPOSE_32_NOIBT_SUBROUTINE_SAFE));;
