...
let mldsa_caddq_mc = define_assert_from_elf "mldsa_caddq_mc" "x86_64/mldsa/mldsa_poly_caddq_avx2_asm.o"
(*** BYTECODE START ***)
[
...
let mldsa_caddq_direct = prove
 (`!x:int32.
    ival(mldsa_caddq x) = mldsa_caddq_spec(ival x)`,
  REWRITE_TAC[mldsa_caddq; mldsa_caddq_spec] THEN BITBLAST_TAC);;
...
let MLDSA_POLY_CADDQ_CORRECT = time prove
 (`!a x pc.
  ...
  TRY(MATCH_MP_TAC MLDSA_CADDQ_SPEC_REM) THEN
  TRY(MATCH_MP_TAC MLDSA_CADDQ_SPEC_LOWER) THEN
  TRY(MATCH_MP_TAC MLDSA_CADDQ_SPEC_UPPER) THEN
  ASM_REWRITE_TAC[]);;
