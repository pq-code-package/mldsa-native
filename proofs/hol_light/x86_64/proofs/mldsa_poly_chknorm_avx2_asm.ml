...
let mldsa_poly_chknorm_mc = define_assert_from_elf "mldsa_poly_chknorm_mc" "x86_64/mldsa/mldsa_poly_chknorm_avx2_asm.o"
(*** BYTECODE START ***)
[
...
let mldsa_poly_chknorm_tmc = define_trimmed "mldsa_poly_chknorm_tmc" mldsa_poly_chknorm_mc;;
let MLDSA_POLY_CHKNORM_TMC_EXEC = X86_MK_CORE_EXEC_RULE mldsa_poly_chknorm_tmc;;
...
let MLDSA_POLY_CHKNORM_CORRECT = prove(
 `!a (x:num->int32) (bound:int32) pc.
  ...
  MAP_UNTIL_TARGET_PC (fun n ->
    X86_STEPS_TAC MLDSA_POLY_CHKNORM_TMC_EXEC [n] THEN
  ...
  (* Now the goal is `bitval(D) = bitval(?i. i<256 /\ abs(ival(x i)) >= ival bound)`.
     Both sides are propositional combinations of 256 atoms; use AArch64 trick. *)
  AP_TERM_TAC THEN
...
let MLDSA_POLY_CHKNORM_NOIBT_SUBROUTINE_CORRECT = prove(
 `!a (x:num->int32) (bound:int32) pc stackpointer returnaddress.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) mldsa_poly_chknorm_tmc /\
                  read RIP s = word pc /\
...
let MLDSA_POLY_CHKNORM_SUBROUTINE_CORRECT = prove(
 `!a (x:num->int32) (bound:int32) pc stackpointer returnaddress.
        ...
        nonoverlapping (stackpointer,8) (a, 1024)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) mldsa_poly_chknorm_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [a; word_zx bound] s /\
             ...
             (\s. read RIP s = returnaddress /\
                  ...
                  read RAX s = word(bitval(~(!i. i < 256 ==> abs(ival(x i)) < ival bound))))
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLDSA_POLY_CHKNORM_NOIBT_SUBROUTINE_CORRECT));;
...
let MLDSA_POLY_CHKNORM_SAFE = time prove
 (full_spec,
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars MLDSA_POLY_CHKNORM_TMC_EXEC);;
...
let MLDSA_POLY_CHKNORM_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a (bound:int32) pc stackpointer returnaddress.
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) mldsa_poly_chknorm_tmc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; word_zx bound] s /\
               ...
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mldsa_poly_chknorm_tmc MLDSA_POLY_CHKNORM_SAFE THEN
...
let MLDSA_POLY_CHKNORM_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e a (bound:int32) pc stackpointer returnaddress.
          ...
          ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) mldsa_poly_chknorm_mc /\
                    read RIP s = word pc /\
                    read RSP s = stackpointer /\
                    read (memory :> bytes64 stackpointer) s = returnaddress /\
                    C_ARGUMENTS [a; word_zx bound] s /\
               ...
               (\s. read RIP s = returnaddress /\
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [a,1024; stackpointer,8]
                                               [a,1024; stackpointer,8]))
               (\s s'. true)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLDSA_POLY_CHKNORM_NOIBT_SUBROUTINE_SAFE));;
