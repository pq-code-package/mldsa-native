...
(* ========================================================================= *)
(* Coefficient layouts used by the RV32 ML-DSA forward NTT proofs.           *)
(* ========================================================================= *)

(* The forward implementation handles two NTT layers in each of four loops.
   This file says where every coefficient is stored after those loops.

   `rv32_mldsa_ntt_phase12`, `rv32_mldsa_ntt_phase123`, and
   `rv32_mldsa_ntt_phase1234` describe the result after two, three, and four
   loops. The `..._INDEX` lemmas translate each loop's row and column counters
   back to a flat coefficient number. The `..._COEFFICIENT_TAC` tactics use
   those translations to read the right value from the preceding loop's
   memory invariant.

   Both forward assembly proofs use these definitions in their loop
   specifications. `mldsa_ntt_arithmetic.ml` later proves that the final
   layout is the mathematical forward NTT. *)

needs "mldsa_native/riscv32/proofs/mldsa_ntt_shared.ml";;

(* The second loop groups coefficients as `64 * g + j + 16 * r`, the third as
   `16 * g + j + 4 * r`, and the fourth writes the flat position
   `4 * g + r`. The combined functions below apply these steps in order. *)
...
(* Translate the second loop's counters `(h,l,r)` to a flat coefficient
   number and back to the coordinates stored after the first loop. *)
...
(* Use the first loop's memory invariant and the index identity above to find
   the coefficient read by one second-loop butterfly. *)
...
(* The loop-entry store may already have updated one coefficient. Rewrite
   that store first, then use the same second-loop lookup. *)
...
(* The third loop uses another three-dimensional view of the same 256 words.
   These identities convert it to the coordinates stored after the second
   loop. *)
...
(* Find the second-loop value used by one third-loop butterfly. *)
...
(* As above, account for the coefficient written while entering the loop
   before consulting the preceding loop's invariant. *)
...
(* The final loop writes flat positions `4 * g + r`. The next lemmas show
   that different groups use different words and provide the lookup tactics
   needed by the loop proof. *)
...
(* Convert a final-loop output position back to the third-loop coordinates
   from which the butterfly reads it. *)
...
(* Handle the loop-entry write before applying the third-loop lookup. *)
