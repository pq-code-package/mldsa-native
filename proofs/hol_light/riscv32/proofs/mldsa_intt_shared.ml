...
(* ========================================================================= *)
(* Common table-loading tactic for the RV32 inverse NTT proofs.              *)
(* ========================================================================= *)

(* Every inverse-transform loop repeatedly loads three `(zeta,w)` pairs and
   moves the table pointer backwards.
   `RV32_MLDSA_INTT_OUTER_SETUP_TAC` proves that assembly section for both the
   fast and shift/add routines. It identifies the six loaded table words,
   proves the new table pointer, and shows that the coefficient array and full
   table description are unchanged.

   The four inverse loops call the tactic with different group counts and
   table positions. The fast and shift/add files also pass different symbolic
   step counts because their instruction sequences have different lengths. *)

(* `group_count` is the number of outer-loop groups, and `table_words` is the
   table position at the start of the loop. `step_count` names the last
   symbolic state in the setup section. `has_t4` records whether that section
   also initializes T4. For example,

     RV32_MLDSA_INTT_OUTER_SETUP_TAC exec 64 510 7 false

   handles the first loop, whose first group consumes table pairs 252--254. *)
...
let RV32_MLDSA_INTT_OUTER_SETUP_TAC
  ...
  let first_read_tm =
    inst
     `read
       (memory :>
        bytes32
         (word_add zetas (word(4 * (next_words - 6 * g + 0))))) s0 =
  ...
  SUBGOAL_THEN
   (inst
     `aligned 4
       (word_add zetas (word(table_bytes - 24 * g)):int32)`)
  ASSUME_TAC THENL
  ...
  SUBGOAL_THEN
   (inst
     `aligned 4
       (word_add zetas (word(4 * (table_words - 6 * g))):int32)`)
  ASSUME_TAC THENL
   [ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
     [CONV_TAC NUM_DIVIDES_CONV;
  ...
  SUBGOAL_THEN
   (inst
     `aligned 4
       (word_add zetas (word(table_bytes - 24 * (g + 1))):int32)`)
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[] THEN
    ASM_SIMP_TAC[ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
  ...
  SUBGOAL_THEN
   (inst
     `word_add
        (word_add zetas (word(4 * (table_words - 6 * g))))
  ...
  SUBGOAL_THEN
   (inst
     `aligned 4
       (word_add zetas (word(4 * (next_words - 6 * g))):int32)`)
  ASSUME_TAC THENL
   [ASM_SIMP_TAC
     [ALIGNED_WORD_ADD_EQ; ALIGNED_WORD; DIMINDEX_32] THEN
    CONJ_TAC THENL
