...
(* ========================================================================= *)
(* Coefficient layouts used by the RV32 ML-DSA inverse NTT proofs.           *)
(* ========================================================================= *)

(* The inverse implementation has four loops that undo two NTT layers at a
   time, followed by one loop that scales all coefficients. This file
   describes the value and position of every coefficient after each loop.

   `rv32_mldsa_intt_phase1`, `rv32_mldsa_intt_phase12`,
   `rv32_mldsa_intt_phase123`, and `rv32_mldsa_intt_phase1234` describe the
   four transform-loop results. `rv32_mldsa_intt_output` adds the final
   scaling. The `..._INDEX` lemmas and lookup tactics translate an assembly
   address back to the preceding loop's coefficient.

   Both inverse assembly proofs use these definitions in their loop
   specifications. `mldsa_intt_arithmetic.ml` proves that the final value is
   the mathematical inverse NTT. *)

needs "mldsa_native/riscv32/proofs/mldsa_ntt_shared.ml";;

(* The first loop starts with adjacent groups of four coefficients and table
   pairs 252--254. Each following loop uses coefficients farther apart and
   moves farther backwards through the table. *)
...
(* The next identities translate each loop's grouped counters back to the
   flat coefficient number and to the coordinates used by the preceding
   loop. *)
...
(* These tactics combine the index identities with the preceding loop's
   memory invariant to recover the word read by the current butterfly. *)
...
(* If the loop-entry store has already changed one word, rewrite that store
   before using the second loop's memory invariant. *)
