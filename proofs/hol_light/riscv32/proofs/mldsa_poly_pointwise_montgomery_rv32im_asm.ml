...
(* This file proves that the RV32 routine replaces each coefficient of `a`
   with its Montgomery product with the corresponding coefficient of `b`.
   `MLDSA_POINTWISE_MONTGOMERY_CORE_CORRECT` proves the result for the main
   loop. `MLDSA_POINTWISE_MONTGOMERY_SUBROUTINE_CORRECT` proves the same
   ...
   preserves the required registers.
   `MLDSA_POINTWISE_MONTGOMERY_SUBROUTINE_SAFE` proves that the function
...
(* The main-loop proof starts after the function has reserved stack space and
   saved S0 and S1. It stops before those registers are restored and the
   function returns. *)

let LENGTH_MLDSA_POINTWISE_MONTGOMERY_MC =
  REWRITE_CONV[mldsa_pointwise_montgomery_mc]
    `LENGTH mldsa_pointwise_montgomery_mc`
  |> CONV_RULE (RAND_CONV LENGTH_CONV);;

let MLDSA_POINTWISE_MONTGOMERY_PREAMBLE_LENGTH = new_definition
  `MLDSA_POINTWISE_MONTGOMERY_PREAMBLE_LENGTH = 12`;;

let MLDSA_POINTWISE_MONTGOMERY_POSTAMBLE_LENGTH = new_definition
  `MLDSA_POINTWISE_MONTGOMERY_POSTAMBLE_LENGTH = 16`;;

let MLDSA_POINTWISE_MONTGOMERY_CORE_START = new_definition
  `MLDSA_POINTWISE_MONTGOMERY_CORE_START =
   MLDSA_POINTWISE_MONTGOMERY_PREAMBLE_LENGTH`;;

let MLDSA_POINTWISE_MONTGOMERY_CORE_END = new_definition
  `MLDSA_POINTWISE_MONTGOMERY_CORE_END =
   LENGTH mldsa_pointwise_montgomery_mc -
   MLDSA_POINTWISE_MONTGOMERY_POSTAMBLE_LENGTH`;;

let MLDSA_POINTWISE_MONTGOMERY_LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV
    [LENGTH_MLDSA_POINTWISE_MONTGOMERY_MC;
     MLDSA_POINTWISE_MONTGOMERY_CORE_START;
     MLDSA_POINTWISE_MONTGOMERY_CORE_END;
     MLDSA_POINTWISE_MONTGOMERY_PREAMBLE_LENGTH;
     MLDSA_POINTWISE_MONTGOMERY_POSTAMBLE_LENGTH] THENC
...
(* Proof outline:
   ...
   2. `RV32_MLDSA_MONTMUL` proves that the multiply, multiply-high, and
      subtraction instructions used for one coefficient equal the word-level
      operation `mldsa_montmul`.
   3. `MLDSA_POINTWISE_MONTGOMERY_CORE_EXACT` follows the 256-iteration loop.
      It proves that each iteration reads `a[i]` and `b[i]`, writes their exact
      word-level Montgomery product to `a[i]`, and leaves unread coefficients
      available to later iterations.
   4. `RV32_MLDSA_MONTMUL_CORRECT` proves the modular result and output bound
      for that word operation. `MLDSA_POINTWISE_MONTGOMERY_CORE_CORRECT`
      applies it to every word produced by the loop.
   5. `MLDSA_POINTWISE_MONTGOMERY_SUBROUTINE_CORRECT` adds the opening
      instructions, closing instructions, and return to the caller.
      `MLDSA_POINTWISE_MONTGOMERY_SUBROUTINE_SAFE` proves that only the
      declared memory is accessed and that the trace does not depend on the
      coefficient values. *)
...
(* First prove the exact word written by the loop, before interpreting that
   word as a modular Montgomery product. *)

let MLDSA_POINTWISE_MONTGOMERY_CORE_EXACT = prove
 (`!a b:int32. !x y:num->int32. !pc:num.
    aligned 4 a /\
    aligned 4 b /\
    nonoverlapping ((word pc):int32,LENGTH
      mldsa_pointwise_montgomery_mc) (a,1024) /\
    nonoverlapping (a,1024) (b,1024)
    ==> ensures riscv
      (\s. aligned_bytes_loaded s (word pc)
             mldsa_pointwise_montgomery_mc /\
           read PC s = word (pc + MLDSA_POINTWISE_MONTGOMERY_CORE_START) /\
           C_ARGUMENTS [a;b] s /\
           (!i. i < 256 ==>
           ...
           (!i. i < 256 ==>
             ...
             y i))
      (\s. read PC s = word (pc + MLDSA_POINTWISE_MONTGOMERY_CORE_END) /\
           !i. i < 256 ==>
             read(memory :> bytes32(word_add a (word(4 * i)))) s =
             mldsa_montmul
               (word_sx (y i),
                word_sx (word_mul (y i) (word 58728449))) (x i))
      (MAYCHANGE
  ...
  CONV_TAC MLDSA_POINTWISE_MONTGOMERY_LENGTH_SIMPLIFY_CONV THEN
  ...
  ENSURES_WHILE_UP_TAC `256` `pc + 32` `pc + 72`
   `\i s.
      ...
      (!j. j < 256
           ==> read (memory :> bytes32 (word_add a (word (4 * j)))) s =
               if j < i then
                 mldsa_montmul
                   (word_sx (y j),
                    word_sx
      ...
      (!j. j < 256
           ==> read (memory :> bytes32 (word_add b (word (4 * j)))) s =
               y j)` THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    SUBGOAL_THEN
     `!j. j < 256
          ==> read (memory :> bytes32 (word_add a (word (4 * j)))) s8 =
              if j < i + 1 then
                mldsa_montmul
                  (word_sx (y j:int32),
                   word_sx
 ...
 (`!a b:int32. !x y:num->int32. !pc:num.
  ...
  RISCV_ADD_RETURN_STACK_TAC
    ...
    (REWRITE_RULE[fst MLDSA_POINTWISE_MONTGOMERY_EXEC]
      (CONV_RULE MLDSA_POINTWISE_MONTGOMERY_LENGTH_SIMPLIFY_CONV
     ...
     ==> ensures riscv
          ...
          (\s. read PC s = returnaddress /\
               exists e2.
                 ...
                 e2 =
                   f_events b a pc
                     (word_sub stackpointer (word 16)) returnaddress /\
