...
(* ========================================================================= *)
(* ML-DSA forward NTT using shift/add multiplication by the modulus.         *)
(* ========================================================================= *)

(* This file proves the RV32 forward NTT that replaces multiplication by the
   modulus with shifts and additions. `MLDSA_NTT_SLOW_CORE_CORRECT` proves
   that the transform loops produce the mathematical forward NTT.
   `MLDSA_NTT_SLOW_SUBROUTINE_CORRECT` proves the same result for a normal
   call to the complete function: it starts at the function entry, returns to
   the caller, restores the stack pointer, and preserves the required
   registers. `MLDSA_NTT_SLOW_SUBROUTINE_SAFE` proves that the function
   accesses only the permitted memory and that its instruction and
   memory-access trace does not depend on the secret coefficients.
...
(**** print_literal_from_elf
 "riscv32/mldsa/mldsa_ntt_rv32im_slowmul_asm.o";;
 ****)

let mldsa_ntt_slowmul_mc = define_assert_from_elf
  "mldsa_ntt_slowmul_mc"
  "riscv32/mldsa/mldsa_ntt_rv32im_slowmul_asm.o"
(*** BYTECODE START ***)
[
...
(* The opening instructions reserve stack space and save S0--S5. The closing
   instructions restore those registers, release the stack space, and return. *)

let LENGTH_MLDSA_NTT_SLOW_MC =
  REWRITE_CONV[mldsa_ntt_slowmul_mc] `LENGTH mldsa_ntt_slowmul_mc`
  |> CONV_RULE (RAND_CONV LENGTH_CONV);;

let MLDSA_NTT_SLOW_PREAMBLE_LENGTH = new_definition
  `MLDSA_NTT_SLOW_PREAMBLE_LENGTH = 28`;;

let MLDSA_NTT_SLOW_POSTAMBLE_LENGTH = new_definition
  `MLDSA_NTT_SLOW_POSTAMBLE_LENGTH = 32`;;

let MLDSA_NTT_SLOW_CORE_START = new_definition
  `MLDSA_NTT_SLOW_CORE_START = MLDSA_NTT_SLOW_PREAMBLE_LENGTH`;;

let MLDSA_NTT_SLOW_CORE_END = new_definition
  `MLDSA_NTT_SLOW_CORE_END =
   LENGTH mldsa_ntt_slowmul_mc - MLDSA_NTT_SLOW_POSTAMBLE_LENGTH`;;

let MLDSA_NTT_SLOW_LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV
    [LENGTH_MLDSA_NTT_SLOW_MC;
     MLDSA_NTT_SLOW_CORE_START; MLDSA_NTT_SLOW_CORE_END;
     MLDSA_NTT_SLOW_PREAMBLE_LENGTH;
     MLDSA_NTT_SLOW_POSTAMBLE_LENGTH] THENC
  NUM_REDUCE_CONV THENC REWRITE_CONV[ADD_0];;

needs "mldsa_native/riscv32/proofs/mldsa_ntt_shared.ml";;
needs "mldsa_native/riscv32/proofs/mldsa_ntt_layout.ml";;
...
(* Proof outline:
   ...
   2. As in the `MUL`/`MULH` routine, four loops handle two NTT layers each.
      `RV32_MLDSA_NTT_SLOW_PHASE1` proves the first loop.
      `RV32_MLDSA_NTT_SLOW_PHASE12`,
      `RV32_MLDSA_NTT_SLOW_PHASE123`, and
      `RV32_MLDSA_NTT_SLOW_PHASE1234` successively add the remaining loops.
      The specifications use the same layout functions as the `MUL`/`MULH`
      proof, so they describe the same two-layer results and memory positions.
   3. The assembly differs inside each Barrett reduction: multiplication by
      q is written as shifts and additions. `RV32_MLDSA_BARRETT_SLOW_PAIR`
      proves that this longer instruction sequence performs the same word
      operation. The first loop is then split into `..._SETUP`, `..._BODY`,
      `..._BACKEDGE`, `..._EXIT`, and `..._LOOP` theorems. The later loops
      use `..._INNER_LOOP`, `..._OUTER_LOOP`, and `..._ENTRY_FULL` because
      they have two counters. The `..._TABLE...` theorems carry the zeta table
      through the coefficient writes.
   4. `RV32_MLDSA_NTT_PHASE1234_CORRECT`, from
      `mldsa_ntt_arithmetic.ml`, turns the final layout into the mathematical
      forward NTT. `MLDSA_NTT_SLOW_CORE_CORRECT` applies that result to the
      memory written by the four assembly loops.
   5. `MLDSA_NTT_SLOW_SUBROUTINE_CORRECT` covers a complete call and return.
      `MLDSA_NTT_SLOW_SUBROUTINE_SAFE` proves the allowed memory accesses and
      a trace that does not depend on the coefficients.

   Most of this file is separate from the `MUL`/`MULH` proof because the
   extra shift/add instructions change the numbered machine states and loop
   boundaries used in the proof. *)

(* ------------------------------------------------------------------------- *)
(* Loop 1: handle the first two NTT layers, with inputs `i + 64 * r`.        *)
(* ------------------------------------------------------------------------- *)

(* Prove the initial table loads and one shift/add butterfly, then lift that
   step through the 64 loop iterations. *)

let RV32_MLDSA_NTT_SLOW_PHASE1_SETUP = prove
 (`!a zetas:int32. !pc.
      aligned 4 zetas
      ==> ensures riscv
           ...
           (MAYCHANGE
            ...
            MAYCHANGE [events])`,
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC; C_ARGUMENTS] THEN
   ...
   (fun th ->
      let _ =
        check
         ...
         th in
      ALL_TAC) THEN
  RISCV_STEPS_TAC MLDSA_NTT_SLOWMUL_EXEC (1--9) THEN
  ENSURES_FINAL_STATE_TAC THEN
  ASM_REWRITE_TAC[] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORD_IWORD_CONV) THEN
  REWRITE_TAC[]);;

let RV32_MLDSA_BARRETT_SLOW_PAIR0 =
  CONV_RULE (DEPTH_CONV WORD_SX_CONV)
   (REWRITE_RULE[LET_DEF; LET_END_DEF]
    (SPECL [`word 4291395073:int32`; `word 2464201481:int32`]
      MLDSA_BARRETT_MUL_SLOW));;

let RV32_MLDSA_BARRETT_SLOW_PAIR1 =
  CONV_RULE (DEPTH_CONV WORD_SX_CONV)
   (REWRITE_RULE[LET_DEF; LET_END_DEF]
    (SPECL [`word 3765607:int32`; `word 1929875198:int32`]
      MLDSA_BARRETT_MUL_SLOW));;

let RV32_MLDSA_BARRETT_SLOW_PAIR2 =
  CONV_RULE (DEPTH_CONV WORD_SX_CONV)
   (REWRITE_RULE[LET_DEF; LET_END_DEF]
...
let RV32_MLDSA_NTT_SLOW_PHASE1_BODY = prove
 (`!a z:int32. !x:num->int32. !i pc.
      ...
      i < 64 /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_ntt_slowmul_mc) (a,1024)
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc)
                  mldsa_ntt_slowmul_mc /\
                read PC s = word(pc + 64) /\
                read S0 s = iword (-- &3572223) /\
                read S1 s = iword (-- &1830765815) /\
                read S2 s = iword (&3765607) /\
                read S3 s = iword (&1929875198) /\
  ...
  MAP_EVERY
   (fun n ->
      RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC []
  ...
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s41" None
   (fun th -> LABEL_TAC "store0" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s42" None
   (fun th -> LABEL_TAC "store1" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s43" None
   (fun th -> LABEL_TAC "store2" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s44" None
   (fun th -> LABEL_TAC "store3" th THEN STRIP_TAC) THEN
  SUBGOAL_THEN
   `!r j. r < 4 /\ j < 64
          ==> read (memory :> bytes32
                 (word_add a (word(4 * (j + 64 * r))))) s44 =
              if j < i + 1 then
                rv32_mldsa_ntt_phase1 x j r
              else x (j + 64 * r)`
  ASSUME_TAC THENL
   [MAP_EVERY X_GEN_TAC [`r:num`; `j:num`] THEN STRIP_TAC THEN
    ...
    ASM_CASES_TAC `j:num = i` THENL
     [FIRST_X_ASSUM SUBST_ALL_TAC THEN
      ...
      REWRITE_TAC
       [RV32_MLDSA_BARRETT_SLOW_PAIR0;
        RV32_MLDSA_BARRETT_SLOW_PAIR1;
        RV32_MLDSA_BARRETT_SLOW_PAIR2] THEN
      ...
      W(fun (asl,w) ->
          ...
          TRANS_TAC EQ_TRANS
           (mk_comb(rator l,`s40:riscvstate`))) THEN
      CONJ_TAC THENL
  ...
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s45" None
   (fun th -> LABEL_TAC "increment" th THEN STRIP_TAC) THEN
  ENSURES_FINAL_STATE_TAC THEN
  ASM_REWRITE_TAC[] THEN
  CONV_TAC WORD_RULE);;

let RV32_MLDSA_NTT_SLOW_PHASE1_BACKEDGE = prove
 (`!a z:int32. !x:num->int32. !i pc.
      0 < i /\ i < 64
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc)
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC[
    ...
    ALL_TAC] THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEPS_TAC MLDSA_NTT_SLOWMUL_EXEC [1] THEN
  ENSURES_FINAL_STATE_TAC THEN
  REPEAT CONJ_TAC THEN FIRST_ASSUM ACCEPT_TAC);;

let RV32_MLDSA_NTT_SLOW_PHASE1_EXIT = prove
 (`!a z:int32. !x:num->int32. !pc.
  ...
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s1" None
   (fun th -> LABEL_TAC "exit_branch" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_NTT_SLOWMUL_EXEC [] "s2" None
...
let RV32_MLDSA_NTT_SLOW_PHASE1_LOOP = prove
 (`!a z:int32. !x:num->int32. !pc.
      aligned 4 a /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_ntt_slowmul_mc) (a,1024)
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `64` `pc + 64` `pc + 244`
   `\i s.
      read S0 s = iword (-- &3572223) /\
      read S1 s = iword (-- &1830765815) /\
      read S2 s = iword (&3765607) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_NTT_SLOW_PHASE1_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`; `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE1_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    MATCH_ACCEPT_TAC
     (SPECL [`a:int32`; `z:int32`; `x:num->int32`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE1_EXIT)]);;

let RV32_MLDSA_NTT_SLOW_PHASE1 = prove
    ...
    MP_TAC
     (MP
       (SPECL [`a:int32`; `zetas:int32`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE1_SETUP)
       (ASSUME `aligned 4 (zetas:int32)`)) THEN
    RISCV_BIGSTEP_TAC MLDSA_NTT_SLOWMUL_EXEC "s1" THENL
      ...
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `word_add zetas (word 24):int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE1_LOOP) THEN
      ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC]]]);;

(* ------------------------------------------------------------------------- *)
(* Loop 2: handle the next two layers, grouped as `64 * g + j + 16 * r`.    *)
(* ------------------------------------------------------------------------- *)

(* Prove one inner butterfly loop, repeat it for the four outer groups, and
   join the result with the first loop. *)

let RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 4
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc)
                ...
                read A0 s = a /\
                read A1 s = word_add zetas (word(24 + 24 * g)) /\
                read T2 s = word_add a (word(256 * g)) /\
                read T3 s = word_add a (word 1024) /\
                wordlist_from_memory(zetas,510) s:int32 list =
                MAP iword rv32_mldsa_ntt_zetas)
           (\s. read PC s = word(pc + 288) /\
                read S0 s = FST(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
                read S1 s = SND(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
                read S2 s = FST(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
                read S3 s = SND(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
                read S4 s = FST(rv32_mldsa_ntt_pair (5 + 3 * g)) /\
                read S5 s = SND(rv32_mldsa_ntt_pair (5 + 3 * g)) /\
                read A0 s = a /\
                read A1 s = word_add zetas (word(24 + 24 * (g + 1))) /\
                read T2 s = word_add a (word(256 * g)) /\
                read T3 s = word_add a (word 1024) /\
      ...
      let read_tm =
        subst
         ...
         `read
           (memory :>
            bytes32
             (word_add zetas (word((24 + 24 * g) + 4 * n)))) s0 =
          iword(EL (6 + 6 * g + n) rv32_mldsa_ntt_zetas)` in
      let address_th =
        WORD_RULE
         (subst
...
let RV32_MLDSA_NTT_SLOW_PHASE2_INNER_BACKEDGE = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE2_INNER_EXIT = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE2_INNER_LOOP = prove
 (`!a z:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 4 /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `16` `pc + 288` `pc + 468`
   `\i s.
      read S0 s = FST(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
      read S1 s = SND(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
      read S2 s = FST(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE2_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE2_INNER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE2_INNER_EXIT) THEN
    ASM_REWRITE_TAC[]]);;

let RV32_MLDSA_NTT_SLOW_PHASE2_INNER_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      ...
      ==> ensures riscv
           (\s.
              (aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
               read PC s = word(pc + 288) /\
               read S0 s = FST(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
               read S1 s = SND(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
               read S2 s = FST(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
   ...
   [MAP_EVERY X_GEN_TAC [`s:riscvstate`; `s':riscvstate`] THEN
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_NTT_SLOW_PHASE2_INNER_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC]]);;

let RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_BACKEDGE = prove
 (`!a zetas:int32. !x:num->int32. !i pc.
      ...
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
                read PC s = word(pc + 476) /\
                read A0 s = a /\
                read A1 s = word_add zetas (word(24 + 24 * i)) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_EXIT = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 476) /\
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEPS_TAC MLDSA_NTT_SLOWMUL_EXEC [1;2] THEN
  ENSURES_FINAL_STATE_TAC THEN
  REPEAT CONJ_TAC THEN
  (FIRST_ASSUM ACCEPT_TAC ORELSE
   ASM_MESON_TAC
...
let RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_BODY = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 4 /\
  ...
  (LABEL_TAC "setup_frame_subsumed") THENL
   [SUBSUMED_MAYCHANGE_TAC; ALL_TAC] THEN
  ENSURES_SEQUENCE_TAC `pc + 288`
   `\s. read S0 s = FST(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
        read S1 s = SND(rv32_mldsa_ntt_pair (3 + 3 * g)) /\
        read S2 s = FST(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
        read S3 s = SND(rv32_mldsa_ntt_pair (4 + 3 * g)) /\
        read S4 s = FST(rv32_mldsa_ntt_pair (5 + 3 * g)) /\
    ...
    MP_TAC
     (MP
       (SPECL
         ...
         RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_SETUP)
       (CONJ
         (ASSUME `aligned 4 (zetas:int32)`)
         (ASSUME `g < 4`))) THEN
    RISCV_BIGSTEP_TAC MLDSA_NTT_SLOWMUL_EXEC "s1" THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE2_INNER_LOOP_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
...
let RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_LOOP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `4` `pc + 256` `pc + 476`
   `\i s.
      read A0 s = a /\
      read A1 s = word_add zetas (word(24 + 24 * i)) /\
      read T2 s = word_add a (word(256 * i)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
       `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
       `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
     ...
     [X_GEN_TAC `s:riscvstate` THEN
      ...
      MATCH_ACCEPT_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_EXIT)]]);;

let RV32_MLDSA_NTT_SLOW_PHASE1_TABLE_PRESERVED = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE1_TABLE_RAW = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `s:riscvstate`; `s':riscvstate`]
         RV32_MLDSA_NTT_SLOW_PHASE1_TABLE_PRESERVED) THEN
      ASM_REWRITE_TAC[];
      USE_THEN "table_before" ACCEPT_TAC];
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`;
       `x:num->int32`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE1) THEN
    ASM_REWRITE_TAC[]]);;


let RV32_MLDSA_NTT_SLOW_PHASE2_ENTRY = prove
 (`!a:int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 252) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE2_ENTRY_FULL = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 252) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE12 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE1_TABLE_RAW) THEN
      ASM_REWRITE_TAC[] THEN
       ...
       [SUBSUMED_MAYCHANGE_TAC;
        RV32_NTT_USE_STRONGER_PRE_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `x:num->int32`; `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE2_ENTRY_FULL)];
         ...
         (SPECL
           [`a:int32`; `zetas:int32`;
            `\n. rv32_mldsa_ntt_phase1
                  x (n MOD 64) (n DIV 64)`;
            `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE2_OUTER_LOOP) THEN
        ASM_REWRITE_TAC[] THEN
        DISCH_THEN RV32_NTT_USE_STRONGER_PRE_TAC]]]);;

(* ------------------------------------------------------------------------- *)
(* Loop 3: handle the next two layers, grouped as `16 * g + j + 4 * r`.     *)
(* ------------------------------------------------------------------------- *)

(* Repeat the nested-loop argument for the third layout, preserving the
   read-only zeta table while coefficients are updated in place. *)

let RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
                ...
                wordlist_from_memory(zetas,510) s:int32 list =
                MAP iword rv32_mldsa_ntt_zetas)
           (\s. read PC s = word(pc + 520) /\

                read S0 s = FST(rv32_mldsa_ntt_pair (15 + 3 * g)) /\
                read S1 s = SND(rv32_mldsa_ntt_pair (15 + 3 * g)) /\
                read S2 s = FST(rv32_mldsa_ntt_pair (16 + 3 * g)) /\
                read S3 s = SND(rv32_mldsa_ntt_pair (16 + 3 * g)) /\
                read S4 s = FST(rv32_mldsa_ntt_pair (17 + 3 * g)) /\
                read S5 s = SND(rv32_mldsa_ntt_pair (17 + 3 * g)) /\
                read A0 s = a /\
      ...
      let read_tm =
        subst
         ...
         `read
           (memory :>
            bytes32
             (word_add zetas (word((120 + 24 * g) + 4 * n)))) s0 =
          iword(EL (30 + 6 * g + n) rv32_mldsa_ntt_zetas)` in
      let address_th =
        WORD_RULE
         (subst
...
let RV32_MLDSA_NTT_SLOW_PHASE3_ENTRY = prove
 (`!a:int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 484) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_BODY = prove
 (`!a z:int32. !x:num->int32. !g j pc.
      aligned 4 a /\
      g < 16 /\
      j < 4 /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_INNER_BACKEDGE = prove
 (`!a z:int32. !x:num->int32. !g i pc.
      g < 16 /\ 0 < i /\ i < 4
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_INNER_EXIT = prove
 (`!a z:int32. !x:num->int32. !g pc.
      g < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_INNER_LOOP = prove
 (`!a z:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 16 /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `4` `pc + 520` `pc + 700`
   `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_INNER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_INNER_EXIT) THEN
    ASM_REWRITE_TAC[]]);;

let RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_BACKEDGE = prove
 (`!a zetas:int32. !x:num->int32. !i pc.
      0 < i /\ i < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_EXIT = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 708) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE3_INNER_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 16 /\
      nonoverlapping
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`;
        ...
        `x:num->int32`; `g:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_INNER_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC]]);;

let RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_BODY = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 16 /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    MP_TAC
     (MP
       (SPECL
         [`a:int32`; `zetas:int32`; `g:num`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_SETUP)
       (CONJ
         (ASSUME `aligned 4 (zetas:int32)`)
         (ASSUME `g < 16`))) THEN
    RISCV_BIGSTEP_TAC MLDSA_NTT_SLOWMUL_EXEC "s1" THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE3_INNER_LOOP_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
      DISCH_THEN(LABEL_TAC "inner_loop") THEN
      MATCH_MP_TAC ENSURES_PREPOSTCONDITION_THM THEN
      USE_THEN "inner_loop"
...
let RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_LOOP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `16` `pc + 488` `pc + 708`
   `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    ENSURES_PRECONDITION_TAC
     `\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
          read PC s = word(pc + 708) /\
     ...
     [X_GEN_TAC `s:riscvstate` THEN
      ...
      MATCH_ACCEPT_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_EXIT)]]);;

let RV32_MLDSA_NTT_SLOW_PHASE3_ENTRY_FULL = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 484) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE123 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_NTT_SLOW_PHASE12) THEN
    ASM_REWRITE_TAC[];
    ENSURES_SEQUENCE_TAC `pc + 488`
     `\s.
          read A0 s = a /\
          read A1 s = word_add zetas (word 120) /\
          read T2 s = a /\
       ...
       [SUBSUMED_MAYCHANGE_TAC;
        RV32_NTT_USE_STRONGER_PRE_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `x:num->int32`; `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE3_ENTRY_FULL)];
        ...
        MP_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            ...
            `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE3_OUTER_LOOP) THEN
        ASM_REWRITE_TAC[] THEN
        DISCH_THEN RV32_NTT_USE_STRONGER_PRE_TAC]]]);;

(* ------------------------------------------------------------------------- *)
(* Loop 4: handle the final two layers and write positions `4 * g + r`.     *)
(* ------------------------------------------------------------------------- *)

(* Prove the final loop and join it with the first three loops. The resulting
   machine theorem describes all 256 output words in flat memory order. *)

let RV32_MLDSA_NTT_SLOW_PHASE4_ENTRY = prove
 (`!a:int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 716) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 64
      ==> ensures riscv
           ...
           (\s. read PC s = word(pc + 748) /\
                ...
                read S0 s = FST(rv32_mldsa_ntt_pair (63 + 3 * g)) /\
                read S1 s = SND(rv32_mldsa_ntt_pair (63 + 3 * g)) /\
                read S2 s = FST(rv32_mldsa_ntt_pair (64 + 3 * g)) /\
                read S3 s = SND(rv32_mldsa_ntt_pair (64 + 3 * g)) /\
                read S4 s = FST(rv32_mldsa_ntt_pair (65 + 3 * g)) /\
                read S5 s = SND(rv32_mldsa_ntt_pair (65 + 3 * g)) /\
                read A0 s = a /\
      ...
      let read_tm =
        subst
         ...
         `read
           (memory :>
            bytes32
             (word_add zetas (word((504 + 24 * g) + 4 * n)))) s0 =
          iword(EL (126 + 6 * g + n) rv32_mldsa_ntt_zetas)` in
      let address_th =
        WORD_RULE
         (subst
...
let RV32_MLDSA_NTT_SLOW_PHASE4_BODY = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE4_BACKEDGE = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE4_EXIT = prove
...
let RV32_MLDSA_NTT_SLOW_PHASE4_BODY_TABLE = prove
   ...
   [MAP_EVERY X_GEN_TAC [`s:riscvstate`; `s':riscvstate`] THEN
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_NTT_SLOW_PHASE4_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC]]);;

let RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_BODY = prove
 (`!a zetas:int32. !x:num->num->num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 64 /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    MP_TAC
     (MP
       (SPECL
         [`a:int32`; `zetas:int32`; `g:num`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_SETUP)
       (CONJ
         (ASSUME `aligned 4 (zetas:int32)`)
         (ASSUME `g < 64`))) THEN
    RISCV_BIGSTEP_TAC MLDSA_NTT_SLOWMUL_EXEC "s1" THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->num->num->int32`; `g:num`; `pc:num`]
         RV32_MLDSA_NTT_SLOW_PHASE4_BODY_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
...
let RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_LOOP = prove
 (`!a zetas:int32. !x:num->num->num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `64` `pc + 720` `pc + 928`
   `\i s.
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`;
        `x:num->num->num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_NTT_SLOWMUL_EXEC];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`;
        `x:num->num->num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE4_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    ENSURES_PRECONDITION_TAC
     `\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
          read PC s = word(pc + 928) /\
     ...
     [X_GEN_TAC `s:riscvstate` THEN
      ...
      MATCH_ACCEPT_TAC
       (SPECL
         ...
         RV32_MLDSA_NTT_SLOW_PHASE4_EXIT)]]);;

let RV32_MLDSA_NTT_SLOW_PHASE4_ENTRY_FULL = prove
 (`!a zetas:int32. !x:num->num->num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
            read PC s = word(pc + 716) /\
...
let RV32_MLDSA_NTT_SLOW_PHASE1234 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`;
        `x:num->int32`; `pc:num`]
       RV32_MLDSA_NTT_SLOW_PHASE123) THEN
    ASM_REWRITE_TAC[];
    ENSURES_SEQUENCE_TAC `pc + 720`
     `\s.
          read A0 s = a /\
          read A1 s = word_add zetas (word 504) /\
          read T2 s = a /\
        ...
        MATCH_ACCEPT_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `rv32_mldsa_ntt_phase123 x`;
            `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE4_ENTRY_FULL)];
        ...
        MATCH_MP_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `rv32_mldsa_ntt_phase123 x`;
            `pc:num`]
           RV32_MLDSA_NTT_SLOW_PHASE4_OUTER_LOOP) THEN
        ASM_REWRITE_TAC[]]]]);;


needs "mldsa_native/riscv32/proofs/mldsa_ntt_arithmetic.ml";;

(* ========================================================================= *)
(* Correctness of the transform loops and the complete callable function.   *)
(* ========================================================================= *)

(* Apply the shared arithmetic theorem to
   `RV32_MLDSA_NTT_SLOW_PHASE1234`. This identifies every output word with
   the mathematical NTT and proves its bound. *)
...
let MLDSA_NTT_SLOW_CORE_CORRECT = prove
 (`!a zetas:int32. !x:num->int32. !pc:num.
    ...
    ==> ensures riscv
      (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
           read PC s = word (pc + MLDSA_NTT_SLOW_CORE_START) /\
           C_ARGUMENTS [a;zetas] s /\
           wordlist_from_memory (zetas,510) s:int32 list =
             MAP iword rv32_mldsa_ntt_zetas /\
           ...
           (!i. i < 256 ==>
             ...
             x i))
      (\s. read PC s = word (pc + MLDSA_NTT_SLOW_CORE_END) /\
           !i. i < 256 ==>
             let zi =
               read(memory :> bytes32(word_add a (word(4 * i)))) s in
             (ival zi == mldsa_bitreverse_forward_ntt (ival o x) i)
               (mod &8380417) /\
  ...
  USE_THEN "machine"
   (fun th ->
     ...
     MAP_EVERY EXISTS_TAC
      [rand(rator(rator tm)); rand(rator tm)]) THEN
  REPEAT CONJ_TAC THENL
   [CONV_TAC MLDSA_NTT_SLOW_LENGTH_SIMPLIFY_CONV THEN
    BETA_TAC THEN REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
 ...
 (`!a zetas:int32. !x:num->int32. !pc:num.
  ...
  RISCV_ADD_RETURN_STACK_TAC
    ...
    (REWRITE_RULE[fst MLDSA_NTT_SLOWMUL_EXEC]
      (CONV_RULE MLDSA_NTT_SLOW_LENGTH_SIMPLIFY_CONV
        MLDSA_NTT_SLOW_CORE_CORRECT))
    `[S0; S1; S2; S3; S4; S5]` 32);;

(* ========================================================================= *)
(* Constant-time and memory-safety proof of the slow forward NTT.            *)
(*                                                                           *)
...
let mldsa_ntt_slow_rv32im_signature =
...
let mldsa_ntt_slow_full_spec,mldsa_ntt_slow_public_vars =
  mk_safety_spec
    ~keep_maychanges:false
    mldsa_ntt_slow_rv32im_signature
    MLDSA_NTT_SLOW_SUBROUTINE_CORRECT
    MLDSA_NTT_SLOWMUL_EXEC;;

let MLDSA_NTT_SLOW_SUBROUTINE_SAFE = time prove
 (`exists f_events.
   forall e a zetas pc stackpointer returnaddress.
     ...
     ALLPAIRS nonoverlapping
       ...
       [word pc,LENGTH mldsa_ntt_slowmul_mc; zetas,2040] /\
     nonoverlapping (a,1024)
       (word_sub stackpointer (word 32),32)
     ==> ensures riscv
          (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_slowmul_mc /\
          ...
          (\s. read PC s = returnaddress /\
               exists e2.
                 ...
                 memaccess_inbounds e2
                   ...
                   [a,1024; word_sub stackpointer (word 32),32])
          (\s s'. true)`,
  ASSERT_CONCL_TAC mldsa_ntt_slow_full_spec THEN
  PROVE_SAFETY_SPEC_TAC
    ~public_vars:mldsa_ntt_slow_public_vars
    MLDSA_NTT_SLOWMUL_EXEC);;
