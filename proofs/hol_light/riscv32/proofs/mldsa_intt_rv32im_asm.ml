...
(* This file proves the RV32 inverse NTT that uses `MUL` and `MULH`.
   `MLDSA_INTT_CORE_CORRECT` proves that the transform loops produce the
   mathematical inverse NTT, including the final scaling step.
   `MLDSA_INTT_SUBROUTINE_CORRECT` proves the same result for a normal call to
   the complete function: it starts at the function entry, returns to the
   caller, restores the stack pointer, and preserves the required registers.
   `MLDSA_INTT_SUBROUTINE_SAFE` proves that the function accesses only the
   permitted memory and that its instruction and memory-access trace does not
   depend on the secret coefficients.
...
(**** print_literal_from_elf
 "riscv32/mldsa/mldsa_intt_rv32im_asm.o";;
 ****)

let mldsa_intt_mc = define_assert_from_elf
  "mldsa_intt_mc" "riscv32/mldsa/mldsa_intt_rv32im_asm.o"
(*** BYTECODE START ***)
[
...
let MLDSA_INTT_PREAMBLE_LENGTH = new_definition
  `MLDSA_INTT_PREAMBLE_LENGTH = 36`;;

let MLDSA_INTT_POSTAMBLE_LENGTH = new_definition
  `MLDSA_INTT_POSTAMBLE_LENGTH = 40`;;
...
let MLDSA_INTT_CORE_END = new_definition
  `MLDSA_INTT_CORE_END =
   LENGTH mldsa_intt_mc - MLDSA_INTT_POSTAMBLE_LENGTH`;;

let MLDSA_INTT_LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV
    [LENGTH_MLDSA_INTT_MC;
...
(* Proof outline:
   ...
   2. The implementation has four separate loops. Each loop undoes two NTT
      layers. `RV32_MLDSA_INTT_PHASE1` proves the loop on adjacent groups of
      four coefficients. `RV32_MLDSA_INTT_PHASE12`,
      `RV32_MLDSA_INTT_PHASE123`, and `RV32_MLDSA_INTT_PHASE1234`
      successively add the three loops with wider coefficient spacing.
      Their specifications use the layout functions from
      `mldsa_intt_layout.ml` and record the zeta-table position after each
      loop. The table is read backwards because the inverse transform uses
      the forward table in reverse order.
   3. The first loop starts with `RV32_MLDSA_INTT_PHASE1_SETUP` and
      `..._OUTER_SETUP`, proves one four-coefficient calculation in
      `..._BODY`, and proves its branch and exit in `..._BACKEDGE` and
      `..._EXIT`. `..._ITERATION` and `..._LOOP` combine those sections.
      The second and third loops use `..._INNER_LOOP` and `..._OUTER_LOOP`
      because they have two counters. The fourth loop uses the simpler
      `..._LOOP` form again. The `..._MEMORY` and `..._TABLE` theorems show
      which coefficient and table facts survive each section.
   4. `RV32_MLDSA_INTT_SCALE` proves the final loop that multiplies all 256
      coefficients by the inverse normalization factor.
      `RV32_MLDSA_INTT_MACHINE` joins that loop to
      `RV32_MLDSA_INTT_PHASE1234`.
   5. `RV32_MLDSA_INTT_PHASE1234_CORRECT`, from
      `mldsa_intt_arithmetic.ml`, proves that the resulting words are the
      mathematical inverse NTT and supplies their bounds.
      `MLDSA_INTT_CORE_CORRECT` applies this result to the machine theorem.
   6. `MLDSA_INTT_SUBROUTINE_CORRECT` adds the opening instructions, closing
      instructions, and return to the caller.
      `MLDSA_INTT_SUBROUTINE_SAFE` proves the allowed memory accesses and a
      trace that does not depend on the coefficients.

   Most of this file records the loop invariants that connect assembly
   addresses with the changing coefficient layout after each pair of layers. *)

(* ------------------------------------------------------------------------- *)
(* Loop 1: undo two layers in adjacent groups `4 * g + r`.                   *)
(* ------------------------------------------------------------------------- *)

(* Prove the initial table position and one inverse butterfly, then lift that
   step through all adjacent groups. *)

let RV32_MLDSA_INTT_PHASE1_SETUP = prove
 (`!a zetas:int32. !pc.
      ensures riscv
       ...
       (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
            ...
            wordlist_from_memory(zetas,510) s:int32 list =
            MAP iword rv32_mldsa_ntt_zetas)
       (MAYCHANGE [PC; T0; A1; T2; T3] ,,
        MAYCHANGE [events])`,
  REWRITE_TAC[fst MLDSA_INTT_EXEC; C_ARGUMENTS] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEPS_TAC MLDSA_INTT_EXEC (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN
  ASM_REWRITE_TAC[] THEN
  SUBGOAL_THEN `read memory s5 = read memory s0` ASSUME_TAC THENL
   [MATCH_MP_TAC
     (ISPECL
       [`memory`; `[PC;T0;A1;T2;T3]`;
        `s0:riscvstate`; `s5:riscvstate`]
     ...
     [REWRITE_TAC[ALL] THEN
      ...
      FIRST_X_ASSUM
       (fun th ->
          if can
              (term_match []
                `(MAYCHANGE [PC] ,, MAYCHANGE [T0] ,,
                  MAYCHANGE [A1] ,, MAYCHANGE [T2] ,,
                  MAYCHANGE [T3]) (s0:riscvstate) s5`)
              (concl th)
          then
...
let RV32_MLDSA_INTT_PHASE1_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 64
      ==> ensures riscv
  ...
  RV32_MLDSA_INTT_OUTER_SETUP_TAC
    MLDSA_INTT_EXEC 64 510 7 false);;

let RV32_MLDSA_INTT_PHASE1_BODY = prove
 (`!a z:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 64 /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024)
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
                read PC s = word(pc + 84) /\
                read T0 s = word 8380417 /\
  ...
  MAP_EVERY
   (fun n ->
      RISCV_STEP_TAC MLDSA_INTT_EXEC []
  ...
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s33" None
   (fun th -> LABEL_TAC "increment" th THEN STRIP_TAC) THEN
  ENSURES_FINAL_STATE_TAC THEN
  ASM_REWRITE_TAC[] THEN
  CONV_TAC WORD_RULE);;

let RV32_MLDSA_INTT_PHASE1_BACKEDGE = prove
 (`!a zetas:int32. !x:num->int32. !i pc.
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC
    ...
    ALL_TAC] THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEPS_TAC MLDSA_INTT_EXEC [1] THEN
...
let RV32_MLDSA_INTT_PHASE1_EXIT = prove
 (`!a zetas:int32. !x:num->int32. !pc.
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s1" None
   (fun th -> LABEL_TAC "exit1" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s2" None
   (fun th -> LABEL_TAC "exit2" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s3" None
...
let RV32_MLDSA_INTT_PHASE1_BODY_TABLE = prove
   ...
   [MAP_EVERY X_GEN_TAC
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_INTT_PHASE1_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_PHASE1_OUTER_SETUP_MEMORY = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 zetas /\
      g < 64
      ==> ensures riscv
   ...
   [MAP_EVERY X_GEN_TAC
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_INTT_PHASE1_OUTER_SETUP) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_PHASE1_ITERATION = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 64 /\
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1_OUTER_SETUP_MEMORY) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
      DISCH_THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1_BODY_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
      DISCH_THEN
       (fun th -> RV32_NTT_USE_STRONGER_PRE_TAC th)]]);;

let RV32_MLDSA_INTT_PHASE1_LOOP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `64` `pc + 56` `pc + 216`
   `\i s.
      read T0 s = word 8380417 /\
      read A0 s = a /\
      read A1 s = word_add zetas (word(2040 - 24 * i)) /\
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `i:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1_ITERATION) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `i:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1_BACKEDGE) THEN
      ASM_REWRITE_TAC[]];
       ...
       [SUBSUMED_MAYCHANGE_TAC;
        MATCH_ACCEPT_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `x:num->int32`; `pc:num`]
           RV32_MLDSA_INTT_PHASE1_EXIT)]]]);;

let RV32_MLDSA_INTT_PHASE1_SETUP_MEMORY = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s.
          (aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
   ...
   [MAP_EVERY X_GEN_TAC
    ...
    MATCH_ACCEPT_TAC
     (SPECL
       ...
       RV32_MLDSA_INTT_PHASE1_SETUP)]);;

let RV32_MLDSA_INTT_PHASE1 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
   ...
   [MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
    ...
    MP_TAC
     (SPECL
       ...
       RV32_MLDSA_INTT_PHASE1_SETUP_MEMORY) THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1_LOOP) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]]);;
...
let RV32_MLDSA_INTT_PHASE2_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 16
      ==> ensures riscv
           ...
           (MAYCHANGE
            ...
            MAYCHANGE [events])`,
  RV32_MLDSA_INTT_OUTER_SETUP_TAC
    MLDSA_INTT_EXEC 16 126 8 true);;

let RV32_MLDSA_INTT_PHASE2_BODY = prove
...
let RV32_MLDSA_INTT_PHASE2_INNER_BACKEDGE = prove
...
let RV32_MLDSA_INTT_PHASE2_INNER_EXIT = prove
 (`!a z:int32. !x:num->int32. !g pc.
      g < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE2_INNER_LOOP = prove
 (`!a z:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 16 /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `4` `pc + 260` `pc + 392`
   `\i s.
      read T0 s = word 8380417 /\
      read S0 s = FST(rv32_mldsa_ntt_pair (60 - 3 * g)) /\
      read S1 s = SND(rv32_mldsa_ntt_pair (60 - 3 * g)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_INNER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_INNER_EXIT) THEN
    ASM_REWRITE_TAC[]]);;

let RV32_MLDSA_INTT_PHASE2_INNER_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 16 /\
      nonoverlapping
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`;
        ...
        `x:num->int32`; `g:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_INNER_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_PHASE2_OUTER_BACKEDGE = prove
 (`!a zetas:int32. !x:num->int32. !i pc.
      0 < i /\ i < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE2_OUTER_EXIT = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
            read PC s = word(pc + 400) /\
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  ENSURES_INIT_TAC "s0" THEN
  RISCV_STEPS_TAC MLDSA_INTT_EXEC [1;2] THEN
  ENSURES_FINAL_STATE_TAC THEN
  REPEAT CONJ_TAC THEN
  (FIRST_ASSUM ACCEPT_TAC ORELSE
   ASM_MESON_TAC
...
let RV32_MLDSA_INTT_PHASE2_OUTER_BODY = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 16 /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    MP_TAC
     (MP
       (SPECL
         [`a:int32`; `zetas:int32`; `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE2_OUTER_SETUP)
       (CONJ
         (ASSUME `aligned 4 (zetas:int32)`)
         (ASSUME `g < 16`))) THEN
    RISCV_BIGSTEP_TAC MLDSA_INTT_EXEC "s1" THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE2_INNER_LOOP_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
...
let RV32_MLDSA_INTT_PHASE2_OUTER_LOOP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `16` `pc + 228` `pc + 400`
   `\i s.
      read T0 s = word 8380417 /\
      read A0 s = a /\
      read A1 s = word_add zetas (word(504 - 24 * i)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_OUTER_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE2_OUTER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    ENSURES_PRECONDITION_TAC
     `\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
          read PC s = word(pc + 400) /\
     ...
     [X_GEN_TAC `s:riscvstate` THEN
      ...
      MATCH_ACCEPT_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_INTT_PHASE2_OUTER_EXIT)]]);;

let RV32_MLDSA_INTT_PHASE12 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
       ...
       (SPECL
         [`a:int32`; `zetas:int32`;
          `\n. rv32_mldsa_intt_phase1
                x (n DIV 4) (n MOD 4)`;
          `pc:num`]
         RV32_MLDSA_INTT_PHASE2_OUTER_LOOP) THEN
      ASM_REWRITE_TAC[] THEN
...
(* Repeat the nested-loop argument at the wider coefficient stride and join
   it with the first two phases. *)

let RV32_MLDSA_INTT_PHASE3_ENTRY_FULL = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
            read PC s = word(pc + 408) /\
  ...
  (FIRST_ASSUM ACCEPT_TAC ORELSE
   ...
   RV32_INTT_PHASE123_UPDATED_COEFFICIENT_TAC));;

let RV32_MLDSA_INTT_PHASE3_OUTER_SETUP = prove
 (`!a zetas:int32. !g pc.
      aligned 4 zetas /\
      g < 4
      ==> ensures riscv
           ...
           (MAYCHANGE
            ...
            MAYCHANGE [events])`,
  RV32_MLDSA_INTT_OUTER_SETUP_TAC
    MLDSA_INTT_EXEC 4 30 8 true);;

let RV32_MLDSA_INTT_PHASE3_BODY = prove
 (`!a z:int32. !x:num->int32. !g j pc.
      aligned 4 a /\
      g < 4 /\
      j < 16 /\
...
let RV32_MLDSA_INTT_PHASE3_INNER_BACKEDGE = prove
 (`!a z:int32. !x:num->int32. !g i pc.
      g < 4 /\ 0 < i /\ i < 16
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE3_INNER_EXIT = prove
 (`!a z:int32. !x:num->int32. !g pc.
      g < 4
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE3_INNER_LOOP = prove
 (`!a z:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 4 /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `16` `pc + 444` `pc + 576`
   `\i s.
      read T0 s = word 8380417 /\
      read S0 s = FST(rv32_mldsa_ntt_pair (12 - 3 * g)) /\
      read S1 s = SND(rv32_mldsa_ntt_pair (12 - 3 * g)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_INNER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `g:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_INNER_EXIT) THEN
    ASM_REWRITE_TAC[]]);;

let RV32_MLDSA_INTT_PHASE3_INNER_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      g < 4 /\
      nonoverlapping
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`;
        ...
        `x:num->int32`; `g:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_INNER_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_PHASE3_OUTER_BACKEDGE = prove
 (`!a zetas:int32. !x:num->int32. !i pc.
      0 < i /\ i < 4
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE3_OUTER_EXIT = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
            read PC s = word(pc + 584) /\
...
let RV32_MLDSA_INTT_PHASE3_OUTER_BODY = prove
 (`!a zetas:int32. !x:num->int32. !g pc.
      aligned 4 a /\
      aligned 4 zetas /\
      g < 4 /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    MP_TAC
     (MP
       (SPECL
         [`a:int32`; `zetas:int32`; `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE3_OUTER_SETUP)
       (CONJ
         (ASSUME `aligned 4 (zetas:int32)`)
         (ASSUME `g < 4`))) THEN
    RISCV_BIGSTEP_TAC MLDSA_INTT_EXEC "s1" THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `x:num->int32`;
          `g:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE3_INNER_LOOP_TABLE) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
      DISCH_THEN(LABEL_TAC "inner_loop") THEN
      MATCH_MP_TAC ENSURES_PREPOSTCONDITION_THM THEN
      USE_THEN "inner_loop"
...
let RV32_MLDSA_INTT_PHASE3_OUTER_LOOP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `4` `pc + 412` `pc + 584`
   `\i s.
      read T0 s = word 8380417 /\
      read A0 s = a /\
      read A1 s = word_add zetas (word(120 - 24 * i)) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_OUTER_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC];
    X_GEN_TAC `i:num` THEN
    STRIP_TAC THEN
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE3_OUTER_BACKEDGE) THEN
    ASM_REWRITE_TAC[];
    ENSURES_PRECONDITION_TAC
     `\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
          read PC s = word(pc + 584) /\
     ...
     [X_GEN_TAC `s:riscvstate` THEN
      ...
      MATCH_ACCEPT_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_INTT_PHASE3_OUTER_EXIT)]]);;

let RV32_MLDSA_INTT_PHASE123 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`; `pc:num`]
       RV32_MLDSA_INTT_PHASE12) THEN
    ASM_REWRITE_TAC[];
    ENSURES_SEQUENCE_TAC `pc + 412`
     `\s. read T0 s = word 8380417 /\
          read A0 s = a /\
          read A1 s = word_add zetas (word 120) /\
          read T2 s = a /\
       ...
       [SUBSUMED_MAYCHANGE_TAC;
        RV32_NTT_USE_STRONGER_PRE_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `x:num->int32`; `pc:num`]
           RV32_MLDSA_INTT_PHASE3_ENTRY_FULL)];
        ...
        MP_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            ...
            `pc:num`]
           RV32_MLDSA_INTT_PHASE3_OUTER_LOOP) THEN
        ASM_REWRITE_TAC[] THEN
...
let RV32_MLDSA_INTT_PHASE4_SETUP = prove
 (`!a zetas:int32. !pc.
      aligned 4 zetas
      ==> ensures riscv
           (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
...
let RV32_MLDSA_INTT_PHASE4_BODY = prove
...
let RV32_MLDSA_INTT_PHASE4_BACKEDGE = prove
...
let RV32_MLDSA_INTT_PHASE4_EXIT = prove
...
let RV32_MLDSA_INTT_PHASE4_LOOP = prove
 (`!a z:int32. !x:num->int32. !pc.
      aligned 4 a /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024)
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `64` `pc + 624` `pc + 756`
   `\i s.
      read T0 s = word 8380417 /\
      read S0 s = FST(rv32_mldsa_ntt_pair 0) /\
      read S1 s = SND(rv32_mldsa_ntt_pair 0) /\
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `z:int32`; `x:num->int32`;
        `i:num`; `pc:num`]
       RV32_MLDSA_INTT_PHASE4_BODY) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC];
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
    EXISTS_TAC
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `z:int32`; `x:num->int32`;
          `i:num`; `pc:num`]
         RV32_MLDSA_INTT_PHASE4_BACKEDGE) THEN
      ASM_REWRITE_TAC[]];
    MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
    EXISTS_TAC
     `MAYCHANGE [PC] ,, MAYCHANGE [events]` THEN
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_ACCEPT_TAC
       (SPECL
         ...
         RV32_MLDSA_INTT_PHASE4_EXIT)]]);;

let RV32_MLDSA_INTT_PHASE4_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024) /\
   ...
   [MAP_EVERY X_GEN_TAC [`s:riscvstate`; `s':riscvstate`] THEN
    ...
    MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`;
        `x:num->int32`; `pc:num`]
       RV32_MLDSA_INTT_PHASE4_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_PHASE4_SETUP_MEMORY = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 zetas
      ==> ensures riscv
           (\s.
    ...
    CONJ_TAC THENL
     [MAP_EVERY X_GEN_TAC
      ...
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`; `pc:num`]
         RV32_MLDSA_INTT_PHASE4_SETUP) THEN
      ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]]);;

let RV32_MLDSA_INTT_PHASE1234 = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC
     (SPECL
       [`a:int32`; `zetas:int32`; `x:num->int32`; `pc:num`]
       RV32_MLDSA_INTT_PHASE123) THEN
    ASM_REWRITE_TAC[];
    ENSURES_SEQUENCE_TAC `pc + 624`
     `\s. read T0 s = word 8380417 /\
          read S0 s = FST(rv32_mldsa_ntt_pair 0) /\
          read S1 s = SND(rv32_mldsa_ntt_pair 0) /\
          read S2 s = FST(rv32_mldsa_ntt_pair 1) /\
       ...
       [SUBSUMED_MAYCHANGE_TAC;
        MP_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            `x:num->int32`; `pc:num`]
           RV32_MLDSA_INTT_PHASE4_SETUP_MEMORY) THEN
        ASM_REWRITE_TAC[] THEN
        DISCH_THEN RV32_NTT_USE_STRONGER_PRE_TAC];
        ...
        MP_TAC
         (SPECL
           [`a:int32`; `zetas:int32`;
            ...
            `pc:num`]
           RV32_MLDSA_INTT_PHASE4_LOOP_TABLE) THEN
        ASM_REWRITE_TAC[] THEN
        DISCH_THEN RV32_NTT_USE_STRONGER_PRE_TAC]]]);;
...
(* Scale every coefficient after the four inverse loops. This supplies the
   normalization required by the mathematical inverse NTT. *)

let RV32_MLDSA_INTT_SCALE_SETUP = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      ensures riscv
       (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
            read PC s = word(pc + 760) /\
  ...
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s4" None
   (fun th -> LABEL_TAC "scale_setup4" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s5" None
   (fun th -> LABEL_TAC "scale_setup5" th THEN STRIP_TAC) THEN
  RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s6" None
...
let RV32_MLDSA_INTT_SCALE_LOOP = prove
 (`!a:int32. !x:num->int32. !pc.
  ...
  REWRITE_TAC[fst MLDSA_INTT_EXEC] THEN
  REPEAT STRIP_TAC THEN
  ENSURES_WHILE_UP_TAC `256` `pc + 784` `pc + 820`
   `\i s.
      read T0 s = word 8380417 /\
      read S6 s = word 16382 /\
      read S7 s = word 16791564 /\
  ...
  REPEAT CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    RISCV_VSTEPS_TAC MLDSA_INTT_EXEC (1--7) THEN
    RISCV_STEP_TAC MLDSA_INTT_EXEC [] "s8" None
     (fun th ->
    ...
    RISCV_STEPS_TAC MLDSA_INTT_EXEC [9] THEN
    ENSURES_FINAL_STATE_TAC THEN
    ASM_REWRITE_TAC
     [rv32_mldsa_intt_output;
      MLDSA_INTT_SCALE_MULH] THEN
    CONV_TAC WORD_RULE;
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    SUBGOAL_THEN
...
let RV32_MLDSA_INTT_SCALE_LOOP_TABLE = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024) /\
   ...
   [MAP_EVERY X_GEN_TAC
    ...
    MATCH_MP_TAC
     (SPECL
       ...
       RV32_MLDSA_INTT_SCALE_LOOP) THEN
    ASM_REWRITE_TAC[fst MLDSA_INTT_EXEC]]);;

let RV32_MLDSA_INTT_SCALE = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      nonoverlapping
        ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024) /\
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_INTT_SCALE_LOOP_TABLE) THEN
      ASM_REWRITE_TAC[] THEN
      DISCH_THEN RV32_NTT_USE_STRONGER_PRE_TAC]]);;

let RV32_MLDSA_INTT_MACHINE = prove
 (`!a zetas:int32. !x:num->int32. !pc.
      aligned 4 a /\
      aligned 4 zetas /\
      nonoverlapping
     ...
     [SUBSUMED_MAYCHANGE_TAC;
      MATCH_MP_TAC
       (SPECL
         [`a:int32`; `zetas:int32`;
          `x:num->int32`; `pc:num`]
         RV32_MLDSA_INTT_PHASE1234) THEN
      ASM_REWRITE_TAC[]];
    MATCH_MP_TAC ENSURES_FRAME_SUBSUMED THEN
    EXISTS_TAC
     `MAYCHANGE
...
(* `RV32_MLDSA_INTT_MACHINE` describes all four transform loops and the final
   scaling loop. Apply the arithmetic result to identify each output word
   with the mathematical inverse NTT and to prove its bound. *)

let MLDSA_INTT_CORE_CORRECT = prove
 (`!a zetas:int32. !x:num->int32. !pc:num.
    aligned 4 a /\
    aligned 4 zetas /\
    nonoverlapping ((word pc):int32,LENGTH mldsa_intt_mc) (a,1024) /\
    nonoverlapping (a,1024) (zetas,2040)
    ==> ensures riscv
      (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
  ...
  USE_THEN "machine"
   (fun th ->
     ...
     MAP_EVERY EXISTS_TAC
      [rand(rator(rator tm)); rand(rator tm)]) THEN
  REPEAT CONJ_TAC THENL
   [CONV_TAC MLDSA_INTT_LENGTH_SIMPLIFY_CONV THEN
    BETA_TAC THEN REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
...
let MLDSA_INTT_SUBROUTINE_CORRECT = prove
 (`!a zetas:int32. !x:num->int32. !pc:num.
    ...
    ALLPAIRS nonoverlapping
      ...
      [((word pc):int32,LENGTH mldsa_intt_mc);
       (zetas,2040)] /\
  ...
  RISCV_ADD_RETURN_STACK_TAC
    ...
    MLDSA_INTT_EXEC
    (REWRITE_RULE[fst MLDSA_INTT_EXEC]
      (CONV_RULE MLDSA_INTT_LENGTH_SIMPLIFY_CONV
        MLDSA_INTT_CORE_CORRECT))
    `[S0; S1; S2; S3; S4; S5; S6; S7]` 32);;

(* ========================================================================= *)
(* Constant-time and memory-safety proof of the fast inverse NTT.            *)
(*                                                                           *)
...
let mldsa_intt_rv32im_signature =
...
let mldsa_intt_full_spec,mldsa_intt_public_vars =
  mk_safety_spec
    ~keep_maychanges:false
    mldsa_intt_rv32im_signature
 ...
 (`exists f_events.
   forall e a zetas pc stackpointer returnaddress.
     ...
     ALLPAIRS nonoverlapping
       ...
       [word pc,LENGTH mldsa_intt_mc; zetas,2040] /\
     nonoverlapping (a,1024)
       (word_sub stackpointer (word 32),32)
     ==> ensures riscv
          (\s. aligned_bytes_loaded s (word pc) mldsa_intt_mc /\
          ...
          (\s. read PC s = returnaddress /\
               exists e2.
                 ...
                 memaccess_inbounds e2
                   ...
                   [a,1024; word_sub stackpointer (word 32),32])
          (\s s'. true)`,
  ASSERT_CONCL_TAC mldsa_intt_full_spec THEN
  PROVE_SAFETY_SPEC_TAC
    ~public_vars:mldsa_intt_public_vars
    MLDSA_INTT_EXEC);;
