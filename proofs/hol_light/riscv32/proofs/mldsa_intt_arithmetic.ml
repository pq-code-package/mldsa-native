...
(* ========================================================================= *)
(* Proof that the four RV32 loops compute the inverse ML-DSA NTT.            *)
(* ========================================================================= *)

(* This file proves the mathematical result described by the inverse layout
   functions.

   `RV32_INTT_RADIX4_SUM_TRANSITION_RV32` proves the arithmetic for one
   four-coefficient calculation. It combines four smaller inverse-transform
   sums into a sum four times as wide. `RV32_INTT_PARTIAL_TRANSITIONS`
   specializes that result for every group used by the four assembly loops
   and saves the resulting 340 theorems.

   `RV32_MLDSA_INTT_PHASE1234_CORRECT` reuses those saved theorems to prove
   that `rv32_mldsa_intt_output` is the mathematical inverse NTT, including
   its final scaling and output bound. Both inverse assembly proofs apply this
   theorem to their final memory state. *)

needs "mldsa_native/riscv32/proofs/mldsa_intt_layout.ml";;
needs "mldsa_native/riscv32/proofs/mldsa_specs_aux.ml";;

(* ========================================================================= *)
(* Arithmetic for one two-layer inverse-NTT step.                            *)
(* ========================================================================= *)

(* First write one inverse butterfly with ordinary integers and list the
   multiplier of each of its four inputs. The next four lemmas match those
   multipliers with the already-proved forward-butterfly root identities. *)
...
(* Split a sum into four consecutive blocks. This is the algebraic step that
   lets one inverse butterfly join four smaller partial transforms. *)
...
(* A partial inverse transform covers `s` consecutive input coefficients.
   One assembly loop combines four such sums and increases the covered width
   from `s` to `4 * s`. *)
...
(* ========================================================================= *)
(* Connecting the 256-term partial sum to the mathematical inverse NTT.     *)
(* ========================================================================= *)

(* After the four transform loops, the helper sum covers all 256 inputs but
   still lacks the inverse-NTT scale. These lemmas rewrite its powers to the
   inverse root and then account for the final scaling loop. *)
...
(* ========================================================================= *)
(* Precomputed arithmetic for every inverse-loop butterfly.                  *)
(* ========================================================================= *)

(* Build the root-product facts needed by every group in the four inverse
   loops. Saving these 340 small theorems avoids proving the same modular
   power identities again during each coefficient-bound calculation. *)
...
(* Precompute bit reversals, root congruences, and the three roots used by
   each butterfly group. Later transition rules can then reuse these small
   theorems instead of evaluating the same powers again. *)
...
(* Prove output `r` of group `g` in loop `p`. Check the three indices, reuse
   the saved table-root facts for the four inputs, and show that this
   butterfly joins the four smaller mathematical sums. *)
...
(* The four loops have 340 butterfly outputs in total. Build each arithmetic
   result once so the final 256-coefficient proof only has to look it up. *)
...
(* ========================================================================= *)
(* Connect the four inverse loops and scaling to the complete inverse NTT.   *)
(* ========================================================================= *)

(* Follow all four layout functions and reuse the saved arithmetic theorem for
   each butterfly. Then apply the scaling result and prove the final modular
   value and bound. Both inverse assembly proofs use the resulting theorem. *)
...
let RV32_MLDSA_INTT_PHASE1234_CORRECT =
  ...
  let build_congbound_groups base conv groups =
    flat
     (map
       (fun terms ->
          let rule = RV32_MEMOIZED_ASM_CONGBOUND_RULE base in
  ...
  let output_expand_conv =
    ...
    DEPTH_CONV RV32_NTT_CLOSED_NUM_CONV in
  let output_rule =
    RV32_MEMOIZED_ASM_CONGBOUND_RULE phase1234_map in
  let output_facts =
    map
