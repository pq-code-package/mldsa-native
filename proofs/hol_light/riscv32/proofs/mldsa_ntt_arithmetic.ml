...
(* ========================================================================= *)
(* Proof that the four RV32 loops compute the forward ML-DSA NTT.            *)
(* ========================================================================= *)

(* This file turns the four concrete coefficient layouts into the
   mathematical forward NTT.

   `RV32_MLDSA_NTT_PHASE1234_PROVE` follows the layout functions from
   `mldsa_ntt_layout.ml`. After each two-layer step it proves, for every
   coefficient, both its value modulo q and an integer bound.
   `RV32_NTT_PARTIAL_256` shows that the starting sum is one input
   coefficient. `RV32_NTT_PARTIAL_1_CONG` shows that, after all eight layers,
   the sum is `mldsa_bitreverse_forward_ntt`.

   `RV32_MLDSA_NTT_PHASE1234_CORRECT` packages these facts in the form used by
   `MLDSA_NTT_CORE_CORRECT` and `MLDSA_NTT_SLOW_CORE_CORRECT`. Keeping this
   arithmetic here lets both assembly proofs share the same mathematical
   argument. *)

needs "mldsa_native/riscv32/proofs/mldsa_ntt_layout.ml";;

(* Turn lower and upper bounds into the absolute-value form used by the
   assembly theorem. *)
...
(* This driver walks through the four layout functions. Its `backend`
   supplies the theorem used to finish the input, each intermediate
   coefficient, and each final output. The chosen backend reuses partial-sum
   theorems instead of expanding the complete 256-term NTT at every step. *)
...
(* ========================================================================= *)
(* Connect the four two-layer steps to the complete forward NTT.             *)
(* ========================================================================= *)

(* The helper sum starts with one input coefficient selected from a
   256-coefficient stride. Each two-layer step divides that stride by four and
   includes four times as many inputs. At stride one, the sum includes all 256
   inputs and is exactly `mldsa_bitreverse_forward_ntt`. *)
...
(* For each word expression, prove its value modulo q and its integer bounds.
   Then name the corresponding mathematical sum, so the next loop can reuse
   that fact without carrying an increasingly large expression. *)
