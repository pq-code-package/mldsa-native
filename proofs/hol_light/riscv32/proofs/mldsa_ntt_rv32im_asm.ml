...
(* This file proves the RV32 forward NTT that uses `MUL` and `MULH`.
   `MLDSA_NTT_CORE_CORRECT` proves that the transform loops produce the
   mathematical forward NTT. `MLDSA_NTT_SUBROUTINE_CORRECT` proves the same
   result for a normal call to the complete function: it starts at the
   function entry, returns to the caller, restores the stack pointer, and
   preserves the required registers. `MLDSA_NTT_SUBROUTINE_SAFE` proves that
   the function accesses only the permitted memory and that its instruction
   and memory-access trace does not depend on the secret coefficients.
...
(**** print_literal_from_elf
 "riscv32/mldsa/mldsa_ntt_rv32im_asm.o";;
 ****)

let mldsa_ntt_mc = define_assert_from_elf
  "mldsa_ntt_mc" "riscv32/mldsa/mldsa_ntt_rv32im_asm.o"
(*** BYTECODE START ***)
[
...
let MLDSA_NTT_PREAMBLE_LENGTH = new_definition
  `MLDSA_NTT_PREAMBLE_LENGTH = 28`;;

let MLDSA_NTT_POSTAMBLE_LENGTH = new_definition
  `MLDSA_NTT_POSTAMBLE_LENGTH = 32`;;
...
let MLDSA_NTT_CORE_END = new_definition
  `MLDSA_NTT_CORE_END =
   LENGTH mldsa_ntt_mc - MLDSA_NTT_POSTAMBLE_LENGTH`;;

let MLDSA_NTT_LENGTH_SIMPLIFY_CONV =
  REWRITE_CONV
    [LENGTH_MLDSA_NTT_MC;
...
(* Proof outline:
   ...
   2. The implementation splits the eight NTT layers into four loops. Each
      loop handles two layers. `RV32_MLDSA_NTT_PHASE1` proves the first loop;
      `RV32_MLDSA_NTT_PHASE12`, `RV32_MLDSA_NTT_PHASE123`, and
      `RV32_MLDSA_NTT_PHASE1234` add the second, third, and fourth loops.
      Their specifications use `mldsa_ntt_layout.ml` to say both what
      two-layer calculation has been completed and where each result is
      stored.
   3. The first loop is split into `RV32_MLDSA_NTT_PHASE1_SETUP`,
      `..._BODY`, `..._BACKEDGE`, and `..._EXIT`; `..._LOOP` combines those
      assembly sections. The later loops use inner and outer loop theorems,
      such as `..._INNER_LOOP` and `..._OUTER_LOOP`, because they have two
      counters. Their `..._ENTRY_FULL` theorems connect one completed loop to
      the next. The `..._TABLE...` theorems carry the unchanged 510-word zeta
      table through these in-place coefficient updates.
   4. `RV32_MLDSA_NTT_PHASE1234_CORRECT`, from
      `mldsa_ntt_arithmetic.ml`, proves that the value described by the four
      layout functions is the mathematical forward NTT and gives its bound.
      `MLDSA_NTT_CORE_CORRECT` applies that result to the final memory state.
   5. `MLDSA_NTT_SUBROUTINE_CORRECT` adds the opening instructions, closing
      instructions, and return to the caller.
      `MLDSA_NTT_SUBROUTINE_SAFE` proves that memory accesses stay in the
      declared regions and that the trace does not depend on the coefficients.

   Most of this file records the loop invariants needed to connect an exact
   assembly address with the corresponding coefficient after each pair of
   layers. *)
...
(* Prove the initial table loads and one butterfly, then lift that step through
   the 64 loop iterations. The resulting theorem records the first output
   layout for the second loop. *)
...
(* Prove one inner butterfly loop, then repeat it for the four outer groups.
   Join the result with the first loop and keep the complete zeta-table
   assertion. *)
...
(* Repeat the nested-loop argument for the third layout, then join it with the
   first two loops. *)
...
(* Prove the final loop and join it with the first three loops. At this point
   the machine theorem describes all 256 output words in flat memory order. *)
...
(* `RV32_MLDSA_NTT_PHASE1234` gives the exact word stored at each output
   position after the four loops. The arithmetic theorem identifies those
   words with the mathematical NTT and supplies the required bounds. *)

let MLDSA_NTT_CORE_CORRECT = prove
 (`!a zetas:int32. !x:num->int32. !pc:num.
    aligned 4 a /\
    aligned 4 zetas /\
    nonoverlapping ((word pc):int32,LENGTH mldsa_ntt_mc) (a,1024) /\
    nonoverlapping (a,1024) (zetas,2040)
    ==> ensures riscv
      (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_mc /\
  ...
  USE_THEN "machine"
   (fun th ->
     ...
     MAP_EVERY EXISTS_TAC
      [rand(rator(rator tm)); rand(rator tm)]) THEN
  REPEAT CONJ_TAC THENL
   [CONV_TAC MLDSA_NTT_LENGTH_SIMPLIFY_CONV THEN
    BETA_TAC THEN REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
 ...
 (`!a zetas:int32. !x:num->int32. !pc:num.
  ...
  RISCV_ADD_RETURN_STACK_TAC
    ...
    (REWRITE_RULE[fst MLDSA_NTT_EXEC]
      (CONV_RULE MLDSA_NTT_LENGTH_SIMPLIFY_CONV
        MLDSA_NTT_CORE_CORRECT))
    `[S0; S1; S2; S3; S4; S5]` 32);;

(* ========================================================================= *)
(* Constant-time and memory-safety proof of the fast forward NTT.            *)
(*                                                                           *)
...
let mldsa_ntt_rv32im_signature =
...
let mldsa_ntt_full_spec,mldsa_ntt_public_vars =
  mk_safety_spec
    ~keep_maychanges:false
    mldsa_ntt_rv32im_signature
 ...
 (`exists f_events.
   forall e a zetas pc stackpointer returnaddress.
     ...
     ALLPAIRS nonoverlapping
       ...
       [word pc,LENGTH mldsa_ntt_mc; zetas,2040] /\
     nonoverlapping (a,1024)
       (word_sub stackpointer (word 32),32)
     ==> ensures riscv
          (\s. aligned_bytes_loaded s (word pc) mldsa_ntt_mc /\
          ...
          (\s. read PC s = returnaddress /\
               exists e2.
                 ...
                 memaccess_inbounds e2
                   ...
                   [a,1024; word_sub stackpointer (word 32),32])
          (\s s'. true)`,
  ASSERT_CONCL_TAC mldsa_ntt_full_spec THEN
  PROVE_SAFETY_SPEC_TAC
    ~public_vars:mldsa_ntt_public_vars
    MLDSA_NTT_EXEC);;
